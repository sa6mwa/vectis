...
int main(void) {
  ...
  assert(config.tls.acme_storage_endpoint == NULL);
  assert(config.tls.acme_storage_namespace == NULL);
  assert(config.tls.acme_storage_key == NULL);
  ...
  config.server.kore_curl_timeout_seconds =
      VECTIS_SERVER_MAX_KORE_CURL_TIMEOUT_SECONDS + 1u;
  app = vectis_app_new(&config, &error);
  assert(app == NULL);
  assert(strstr(error.message, "kore_curl_timeout_seconds") != NULL);
