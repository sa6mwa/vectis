...
static int run_lua_contract(const char *endpoint_url, const char *script_path,
  ...
  static const unsigned char script[] =
      ...
      "local default_server = assert(opcua.server())\n"
      "assert(default_server:close() == true)\n"
      "local coroutine_server\n"
      "local owner_thread = coroutine.create(function()\n"
      "  coroutine_server = assert(opcua.server())\n"
      "  assert(coroutine_server:set_access_control({ handler = function() "
      "return true end }) == true)\n"
      "end)\n"
      "assert(coroutine.resume(owner_thread))\n"
      "owner_thread = nil; collectgarbage('collect')\n"
      "assert(coroutine_server:close() == true)\n"
      "coroutine_server = nil; collectgarbage('collect')\n"
      ...
      "local closing_client\n"
      "local callback_thread = coroutine.create(function()\n"
      "  closing_client = assert(opcua.connect(OPCUA_ENDPOINT))\n"
      "  assert(type(closing_client:read_async(node, function() "
      "error('callback must remain valid during client close') end)) == "
      "\"number\")\n"
      "end)\n"
      "assert(coroutine.resume(callback_thread))\n"
      "callback_thread = nil; collectgarbage('collect')\n"
      "assert(closing_client:close() == true)\n"
      "closing_client = nil; collectgarbage('collect')\n"
      "local weak_owners = setmetatable({}, { __mode = 'v' })\n"
      "local abandoned_thread = coroutine.create(function()\n"
      "  local abandoned_client = assert(opcua.client())\n"
      "  weak_owners.thread = coroutine.running()\n"
      "  weak_owners.client = abandoned_client\n"
      "  coroutine.yield()\n"
      "end)\n"
      "assert(coroutine.resume(abandoned_thread))\n"
      "abandoned_thread = nil\n"
      "collectgarbage('collect'); collectgarbage('collect')\n"
      "assert(weak_owners.thread == nil and weak_owners.client == nil)\n"
