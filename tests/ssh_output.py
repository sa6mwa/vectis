...
def main():
    ...
    for mode in ("stderr", "mixed", "delayed", "empty", "idle"):
        errors = []
        with socket.socket() as listener:
            listener.bind(("127.0.0.1", 0))
            listener.listen(1)
            listener.settimeout(15)
            thread = threading.Thread(target=serve, args=(listener, key, errors))
            thread.start()
            started = time.monotonic()
            try:
                result = subprocess.run(
                    [str(binary), str(script.resolve()),
                     str(listener.getsockname()[1]), fingerprint, mode],
