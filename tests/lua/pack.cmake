...
execute_process(COMMAND "${VECTIS_BIN}" -a pack --invalid-pack-option ignored --script "${script}" --output "${invalid_option_output}"
                RESULT_VARIABLE invalid_option_result
                OUTPUT_VARIABLE invalid_option_stdout
                ERROR_VARIABLE invalid_option_stderr)
if(invalid_option_result EQUAL 0)
  message(FATAL_ERROR "invalid pack option unexpectedly succeeded")
endif()
if(NOT invalid_option_stderr MATCHES "unknown pack argument: --invalid-pack-option")
  message(FATAL_ERROR "invalid pack option failed with unexpected error: ${invalid_option_stdout}${invalid_option_stderr}")
endif()
...
file(MAKE_DIRECTORY "${asset_dir}/assets/empty" "${asset_dir}/vectis")
file(WRITE "${asset_dir}/index.html" "<!doctype html><title>Acme Test</title>\n")
file(WRITE "${asset_dir}/vectis/index.html" "<!doctype html><title>Nested Acme Test</title>\n")
...
file(WRITE "${asset_dir}/assets/rows.csv" "id,count\npacked-alpha,7\npacked-beta,8\n")
...
file(WRITE "${asset_manifest_template}" "<!doctype html><title>Vectis login</title><main data-template=\"username\">{{content}}</main>\n")
file(WRITE "${asset_file_login_template}" "<!doctype html><title>Vectis login</title><main data-template=\"username\">{{content}}</main>\n")
...
string(REPLACE
       "assert(served_root.body:match(\"Acme Test\"))"
       "assert(served_root.body:match(\"Acme Test\"))\nlocal embedded_directory_redirect = curl.perform({url = \"http://127.0.0.1:28181/vectis\", protocols = \"http\", follow_redirects = false, timeout_ms = 2000, connect_timeout_ms = 1000, no_signal = true})\nassert(embedded_directory_redirect.ok == true, embedded_directory_redirect.error)\nassert(embedded_directory_redirect.status == 308)\nassert(embedded_directory_redirect.headers:lower():find(\"location: /vectis/\", 1, true))\nlocal served_nested_directory = fetch_http(\"/vectis/\")\nassert(served_nested_directory.status == 200)\nassert(served_nested_directory.body:match(\"Nested Acme Test\"))\nlocal served_nested_index = fetch_http(\"/vectis/index.html\")\nassert(served_nested_index.status == 200)\nassert(served_nested_index.body:match(\"Nested Acme Test\"))"
...
string(REPLACE
       ...
       "local missing, err = vectis.embedded.read(\"/missing.txt\")\nassert(missing == nil)\nassert_status_error(err, vectis.ERR_INVALID, \"embedded asset not found\")\nlocal missing_stat, missing_stat_err = vectis.embedded.stat(\"/missing.txt\")\nassert(missing_stat == nil)\nassert_status_error(missing_stat_err, vectis.ERR_INVALID, \"embedded asset not found\")\nlocal missing_chunks, missing_chunks_err = vectis.embedded.chunks(\"/missing.txt\", 4)\nassert(missing_chunks == nil)\nassert_status_error(missing_chunks_err, vectis.ERR_INVALID, \"embedded asset not found\")"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "assert(extract_err:match(\"embedded asset already exists\"))"
       "assert_status_error(extract_err, vectis.ERR_CONFLICT, \"embedded asset already exists\")"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "assert(verify_err:match(\"embedded asset verification failed\"))"
       "assert_status_error(verify_err, vectis.ERR_CONFLICT, \"embedded asset verification failed\")"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "assert(missing_verify_err:match(\"embedded asset is missing\"))"
       "assert_status_error(missing_verify_err, vectis.ERR_CONFLICT, \"embedded asset is missing\")"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "assert(vectis.embedded.default_extract_policy() == \"repair\")"
       "assert(vectis.embedded.default_extract_policy() == \"repair\")\nlocal tree_sha = vectis.embedded.tree_sha256()\nassert(#tree_sha == 64)\nassert(tree_sha:match(\"^[0-9a-f]+$\"))"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "end\nlocal function write_file(path, body)"
...
string(REPLACE
       "assert(vectis.embedded.read(\"/templates/login.html\"):match(\"username\"))"
       "assert(vectis.embedded.read(\"/templates/login.html\"):match(\"username\"))\nlocal dsv_schema = lonejson.schema(\"packed-dsv-row\", {\n  lonejson.field(\"id\", lonejson.string({required = true})),\n  lonejson.field(\"count\", lonejson.i64({required = true})),\n})\nassert(vectis.embedded.stat(\"/assets/rows.csv\").kind == \"file\")\nlocal dsv_rows = assert(dsv.parse({schema = dsv_schema, data = vectis.embedded.read(\"/assets/rows.csv\")}))\nassert(#dsv_rows == 2)\nassert(dsv_rows[1].id == \"packed-alpha\")\nassert(dsv_rows[2].count == 8)\nlocal dsv_total = 0\nassert(dsv.each({schema = dsv_schema, data = vectis.embedded.read(\"/assets/rows.csv\"), on_row = function(_, row) dsv_total = dsv_total + row.count end}) == true)\nassert(dsv_total == 15)"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "assert(app_txt_sha:match(\"^[0-9a-f]+$\"))"
...
string(REPLACE
       "assert(read_file(extract_to .. \"/templates/login.html\"):match(\"username\"))\nassert(vectis.embedded.extract({to = extract_to, policy = \"verify\"}) == true)"
       "assert(read_file(extract_to .. \"/templates/login.html\"):match(\"username\"))\nassert(path_exists(extract_to .. \"/assets\"))\nassert(path_exists(extract_to .. \"/assets/empty\"))\nlocal disk_server = assert(vectis.app.new({bind = \"127.0.0.1\", port = 28183}))\nassert(disk_server:static_directory({path_prefix = \"/disk\", root_dir = extract_to, content_type = \"text/plain\", index_file = \"index.html\", index = true}) == true)\nassert(disk_server:start() == true)\nlocal disk_index\nfor _ = 1, 20 do\n  disk_index = curl.perform({url = \"http://127.0.0.1:28183/disk\", protocols = \"http\", timeout_ms = 2000, connect_timeout_ms = 1000, no_signal = true})\n  if disk_index.ok then break end\n  assert(vectis.sleep_ms(100) == true)\nend\nassert(disk_index.ok == true, disk_index.error)\nassert(disk_index.status == 200)\nassert(disk_index.body:match(\"Acme Test\"))\nlocal disk_asset = curl.perform({url = \"http://127.0.0.1:28183/disk/assets/app.txt\", protocols = \"http\", timeout_ms = 2000, connect_timeout_ms = 1000, no_signal = true})\nassert(disk_asset.ok == true, disk_asset.error)\nassert(disk_asset.status == 200)\nassert(disk_asset.body == \"generic embedded asset\\n\")\nassert(disk_server:stop() == true)\ndisk_server:close()\nassert(vectis.embedded.extract({to = extract_to, policy = \"verify\"}) == true)"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "assert(disk_index.body:match(\"Acme Test\"))"
       "assert(disk_index.body:match(\"Acme Test\"))\nlocal disk_trailing_index = curl.perform({url = \"http://127.0.0.1:28183/disk/\", protocols = \"http\", timeout_ms = 2000, connect_timeout_ms = 1000, no_signal = true})\nassert(disk_trailing_index.ok == true, disk_trailing_index.error)\nassert(disk_trailing_index.status == 200, tostring(disk_trailing_index.status) .. ' ' .. tostring(disk_trailing_index.body))\nassert(disk_trailing_index.body:match(\"Acme Test\"))"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "disk_index = curl.perform({url = \"http://127.0.0.1:28183/disk\", protocols = \"http\", timeout_ms = 2000, connect_timeout_ms = 1000, no_signal = true})"
       "disk_index = curl.perform({url = \"http://127.0.0.1:28183/disk\", protocols = \"http\", follow_redirects = false, timeout_ms = 2000, connect_timeout_ms = 1000, no_signal = true})"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "assert(disk_index.status == 200)\nassert(disk_index.body:match(\"Acme Test\"))"
       "assert(disk_index.status == 308)\nassert(disk_index.headers:lower():find(\"location: /disk/\", 1, true))"
       asset_script_body "${asset_script_body}")
string(REPLACE
       "local webdav_credentials = [[${asset_webdav_credentials}]]"
       "local webdav_credentials = [[${asset_webdav_credentials}]]\nlocal https_cert_bundle = [[${asset_https_cert_bundle}]]"
...
string(REPLACE
       ...
       asset_script_body "${asset_script_body}")
string(REPLACE
       "obsolete webdav logout fixture"
