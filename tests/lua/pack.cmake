...
file(REMOVE "${asset_codesign_mode_required_output}")
execute_process(COMMAND "${VECTIS_BIN}" -a pack --script "${script}" --output "${asset_codesign_mode_required_output}" --hardened-runtime --timestamp --entitlements "${pack_entitlements}"
                RESULT_VARIABLE codesign_mode_required_result
                OUTPUT_VARIABLE codesign_mode_required_stdout
                ERROR_VARIABLE codesign_mode_required_stderr)
if(codesign_mode_required_result EQUAL 0)
  message(FATAL_ERROR "pack signing option without signing mode unexpectedly succeeded")
endif()
if(EXISTS "${asset_codesign_mode_required_output}")
  message(FATAL_ERROR "pack signing option without signing mode created an output artifact")
endif()
if(NOT codesign_mode_required_stderr MATCHES "--hardened-runtime, --timestamp, and --entitlements require --codesign or --ad-hoc-codesign")
  message(FATAL_ERROR "pack signing option without signing mode failed with unexpected error: ${codesign_mode_required_stdout}${codesign_mode_required_stderr}")
endif()

file(REMOVE "${asset_codesign_missing_entitlements_output}" "${missing_pack_entitlements}")
execute_process(COMMAND "${VECTIS_BIN}" -a pack --script "${script}" --output "${asset_codesign_missing_entitlements_output}" --ad-hoc-codesign --entitlements "${missing_pack_entitlements}"
                RESULT_VARIABLE codesign_missing_entitlements_result
                OUTPUT_VARIABLE codesign_missing_entitlements_stdout
                ERROR_VARIABLE codesign_missing_entitlements_stderr)
if(codesign_missing_entitlements_result EQUAL 0)
  message(FATAL_ERROR "pack missing entitlements unexpectedly succeeded")
endif()
if(EXISTS "${asset_codesign_missing_entitlements_output}")
  message(FATAL_ERROR "pack missing entitlements created an output artifact")
endif()
if(NOT codesign_missing_entitlements_stderr MATCHES "failed to read entitlements file")
  message(FATAL_ERROR "pack missing entitlements failed with unexpected error: ${codesign_missing_entitlements_stdout}${codesign_missing_entitlements_stderr}")
endif()
