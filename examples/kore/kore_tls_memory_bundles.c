...
int main(void) {
  ...
  const char server_bundle[] =
      ...
      "-----END PRIVATE KEY-----\n";
  const char client_ca_bundle[] =
      "-----BEGIN CERTIFICATE-----\n"
      "client ca certificate pem\n"
      "-----END CERTIFICATE-----\n";
  const char upstream_ca_bundle[] =
      "-----BEGIN CERTIFICATE-----\n"
      "ca certificate pem\n"
      "-----END CERTIFICATE-----\n";
  ...
  config.tls.cert_key_bundle = vectis_source_from_memory(server_bundle,
                                                         sizeof(server_bundle) - 1u);
  config.tls.ca_bundle = vectis_source_from_memory(upstream_ca_bundle,
                                                  sizeof(upstream_ca_bundle) - 1u);
