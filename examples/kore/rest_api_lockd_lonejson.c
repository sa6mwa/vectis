...
static const lonejson_field order_request_fields[] = {
    ...
    LONEJSON_FIELD_STRING_FIXED_REQ(order_request, status, "status",
                                    LONEJSON_OVERFLOW_FAIL)};

static const lonejson_field order_response_fields[] = {
    LONEJSON_FIELD_STRING_FIXED_REQ(order_response, id, "id",
                                    LONEJSON_OVERFLOW_FAIL),
    LONEJSON_FIELD_STRING_FIXED_REQ(order_response, saved, "saved",
                                    LONEJSON_OVERFLOW_FAIL)};
...
static vectis_status save_order(vectis_app *app, vectis_request *request,
  ...
  if (status != VECTIS_OK) {
    return vectis_response_error_json(response, 400, "invalid_order_id",
                                      "order id cannot be used as a lockd key",
                                      error != NULL ? error->message : "",
                                      error);
  }
  status = vectis_lockd_state_save(lockd, key, "orders-api", 30L,
                                   &order_request_map, &input, error);
  if (status != VECTIS_OK) {
    return vectis_response_error_json(
        response, 503, "order_save_failed", "failed to save order state",
  ...
  return vectis_response_json(response, 200, &order_response_map, &output,
                              error);
}
...
int main(void) {
  ...
  const char bundle[] = "-----BEGIN CERTIFICATE-----\n"
                        "example\n"
                        "-----END CERTIFICATE-----\n";
  ...
  config.lockd.client_bundle =
      vectis_source_from_memory(bundle, sizeof(bundle) - 1u);
