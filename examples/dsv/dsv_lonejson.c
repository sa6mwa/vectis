...
static const lonejson_field order_row_fields[] = {
    LONEJSON_FIELD_STRING_FIXED_REQ(order_row, id, "id",
                                    LONEJSON_OVERFLOW_FAIL),
    LONEJSON_FIELD_STRING_FIXED_REQ(order_row, customer, "customer",
                                    LONEJSON_OVERFLOW_FAIL),
...
static vectis_status on_order_row(void *userdata, size_t row_number, void *row,
                                  vectis_error *error) {
  order_totals *totals;
  order_row *order;
  ...
  printf("row=%lu id=%s customer=%s quantity=%ld priority=%d\n",
         (unsigned long)row_number, order->id, order->customer,
         (long)order->quantity, order->priority);
  return VECTIS_OK;
}

int main(int argc, char **argv) {
  const char fallback_csv[] = "# ordinary row-only CSV comment\n"
  ...
  vectis_error error;
  order_totals totals;
  order_row rows[2] = {{"ord-2001", "globex,west", 8, 1},
  ...
  source = argc > 1 ? vectis_source_from_path(argv[1])
                    : vectis_source_from_memory(fallback_csv,
                                                sizeof(fallback_csv) - 1u);

  status = vectis_dsv_parse_lonejson_source(&source, &order_row_map, &csv,
                                            on_order_row, &totals, &error);
  if (status != VECTIS_OK) {
  ...
  source = argc > 1 ? vectis_source_from_path(argv[1])
                    : vectis_source_from_memory(fallback_csv,
                                                sizeof(fallback_csv) - 1u);
  status = vectis_dsv_source_to_lonejson_array(&source, &order_row_map, &csv,
                                               &json, &error);
  if (status != VECTIS_OK) {
  ...
  printf("rows=%lu quantity=%ld priority_rows=%d\n", (unsigned long)totals.rows,
         (long)totals.quantity, totals.priority_rows);
  printf("%.*s\n", (int)json.size, (const char *)json.data);

  csv = vectis_dsv_csv();
  csv.comment_prefix = "#";
  status = vectis_dsv_lonejson_rows_to_bytes(&order_row_map, &csv, rows, 2u, 0u,
                                             &export_csv, &error);
  if (status != VECTIS_OK) {
