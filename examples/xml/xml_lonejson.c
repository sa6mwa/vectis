...
static const lonejson_field invoice_line_fields[] = {
    LONEJSON_FIELD_STRING_FIXED_REQ(invoice_line, sku, "sku",
                                    LONEJSON_OVERFLOW_FAIL),
    LONEJSON_FIELD_I64_REQ(invoice_line, quantity, "quantity")};

static const lonejson_field money_fields[] = {
    LONEJSON_FIELD_STRING_FIXED_REQ(money, currency, "currency",
                                    LONEJSON_OVERFLOW_FAIL),
...
static const lonejson_field invoice_fields[] = {
    ...
    LONEJSON_FIELD_OBJECT_ARRAY(invoice, line, "line", invoice_line,
                                &invoice_line_map, LONEJSON_OVERFLOW_FAIL),
...
int main(void) {
  ...
  status = vectis_xml_parse_lonejson_source(&source, &invoice_map, &config,
                                            &doc, &error);
  if (status != VECTIS_OK) {
  ...
  printf("%s %.2f %s %lu\n", doc.id, doc.amount.text, doc.amount.currency,
