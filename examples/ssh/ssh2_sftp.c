...
int main(void) {
  ...
  local_upload =
      env_or_default("VECTIS_SSH_SFTP_UPLOAD_FILE", "ssh-sftp-upload.txt");
  local_download =
      env_or_default("VECTIS_SSH_SFTP_DOWNLOAD_FILE", "ssh-sftp-download.txt");
  remote_path = env_or_default("VECTIS_SSH_SFTP_REMOTE_FILE",
                               "/config/ssh-sftp-upload.txt");
  ...
  if (ssh->sftp_upload_file(ssh, local_upload, remote_path, &error) !=
      VECTIS_OK) {
    ssh->close(ssh);
    return print_error("ssh->sftp_upload_file", &error);
  }
  if (ssh->sftp_download_file(ssh, remote_path, local_download, &error) !=
      VECTIS_OK) {
    ssh->close(ssh);
  ...
  if (!file_equals(local_download, payload)) {
    fprintf(stderr,
            "downloaded SSH SFTP file did not match uploaded payload\n");
    ssh->close(ssh);
    return 1;
  }
  ssh->close(ssh);
