...
- [ ] Complete the dependency-native OPC UA Lua server facade over all remaining relevant `cpkt_opcua_server_*` APIs, including security config, access-control callbacks, methods, method argument metadata, PubSub/MQTT configuration, event monitoring workflows, long-lived callback ownership machinery, and documented C-only native-pointer exclusions.
