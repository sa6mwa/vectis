...
- [x] Extend Vectis service-friendly lockd helpers only where they reduce real C workflow friction without obscuring the public `liblockdc` 0.14.0 API: C consumers now have bounded `vectis_lockd_consumer_json_into()` payload-to-lonejson decoding, while direct `liblockdc` remains the queue/ack/nack/retry authority.
...
- [x] Implement the documented runtime phase/order contract: validation before materialization, T2 child readiness before supervisor service materialization, service class start/stop ordering, rollback on partial startup failure, and explicit route-backed managed `start()` supervision semantics.
- [x] Add focused C runtime tests for the phase/order contract across metrics, generic managed workers, lockdc consumer services, and route-backed no-service managed starts.
...
- [x] Define the OPC UA Lua callback, ownership, and Vectis concurrency contract: Lua callbacks belong to one owning Lua state, run only during explicit owner-state pumps, and background Kore/lockd-style services must stay C-owned unless their Lua callbacks are pumped on that owner state.
- [x] Add OPC UA Lua server foundation bindings for server construction, endpoint setup, identity, simple access control, namespaces, object/variable creation, read/write, startup/iterate/shutdown, endpoint URL, close/finalizers, smoke coverage, direct runtime e2e, and packed example e2e.
- [x] Add OPC UA Lua server node-model, reference, deletion, and common attribute bindings for type/view creation, references, node identity/class/name metadata, display/description/write-mask, abstract/symmetric/inverse/view/event/access-level/value-rank/sampling/historizing/executable attributes, with direct and packed local e2e coverage.
- [x] Add OPC UA Lua server data-value, full array value/read families, index-range writes, browse-path translation, and prepared event helpers, with smoke coverage plus direct and packed local e2e coverage.
- [x] Add OPC UA Lua server/client materialized browse and paged browse result bindings over `cpkt_opcua_*_browse_children*` and `cpkt_opcua_*_browse_next`, with direct and packed local e2e coverage.
- [x] Add OPC UA Lua server method registration/callback retention, server/client method argument metadata reads, and sync client method-call bindings with direct and packed local e2e coverage.
- [x] Add OPC UA Lua server PubSub/MQTT configuration wrappers for disabled deterministic MQTT connection graphs, published datasets/variables, writer/reader groups, data-set writers/readers, and PubSub byte-string save/load structured error behavior with direct and packed local e2e coverage.
- [x] Add OPC UA Lua server access-control callback retention over `cpkt_opcua_server_set_access_control_callback`, including denied/accepted credential e2e coverage through `opcua.connect()`.
- [x] Add OPC UA Lua client/server default security byte-material configuration over `cpkt_opcua_client_set_default_encryption()` and `cpkt_opcua_server_set_default_security()`, with explicit no-hidden-file-read docs and validation smoke coverage.
- [x] Document OPC UA Lua native-pointer C-only exclusions for `cpkt_opcua_*_native*`, native config/plugin, native variant/data-value, PubSub/history native escape hatches, and lifecycle assertions so the Lua facade does not leak borrowed open62541 pointers.
- [x] Complete the dependency-native OPC UA Lua server facade over all non-native `cpkt_opcua_server_*` APIs, including event creation/triggering workflows, owner-state callback contracts, and server PubSub/MQTT graph wrappers; lifecycle audit verifies only documented native-pointer exclusions remain.
- [x] Add OPC UA Lua client synchronous data-value, integer array/range, index-range write, browse-path translation, and basic remote object/variable creation/deletion bindings with direct and packed local e2e coverage.
- [x] Add OPC UA Lua client full array-family read/range bindings for boolean, integer, double, string, byte-string, uint64, datetime, status, GUID, qualified-name, and localized-text arrays with direct and packed local e2e coverage.
- [x] Add OPC UA Lua client common attribute and remote node-management parity for type/view creation, references, node identity/class/name metadata, display/description/write-mask, abstract/symmetric/inverse/view/event/access-level/value-rank/sampling/historizing/executable attributes, with direct and packed local e2e coverage.
...
- [x] Add Lua examples for CAI live/service-adapter workflows as those facades land.
...
- [x] Add Lua unit and end-to-end tests for every Lua facade, including preload smoke, method-call DX, ownership/finalizer cleanup, structured error objects, packed execution, large-value streaming/spooling, and local deterministic protocol scenarios.
...
- [x] Use one self-describing appended trailer with magic, lengths, hashes, and metadata on Linux; packed output always has the input executable's architecture.
- [x] Keep packing self-contained: no target selection, SDK, relink, CMake, signing, or external tools.
- [x] Disable pack creation and packed-payload startup on Darwin because appending bytes invalidates the Mach-O code signature required on Apple Silicon.
...
- [x] Keep the pack action tool-free by rejecting it on Darwin rather than invoking signing, notarization, entitlement, or Gatekeeper tooling.
...
- [x] Run the same Codex review command mandated by the lifecycle skill and
  ...
  during fix iterations, then the release-gate form
  `codex review -c model=gpt-5.6-sol -c model_reasoning_effort=medium --base <release-branch>`
  before release. Current branch gate completed with
  `codex review -c model=gpt-5.6-sol -c model_reasoning_effort=medium --base origin/trunk`.
