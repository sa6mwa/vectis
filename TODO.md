...
- [ ] Complete the dependency-native OPC UA Lua server facade over all remaining relevant `cpkt_opcua_server_*` APIs, including security config, access-control callbacks, live PubSub/MQTT configuration validation, event monitoring workflows, async-safe callback queueing, and documented C-only native-pointer exclusions.
