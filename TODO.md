...
- [ ] Complete the dependency-native OPC UA Lua server facade over all remaining relevant `cpkt_opcua_server_*` APIs, including security config, access-control callbacks, methods, browse callbacks/pagination, method argument metadata, PubSub/MQTT configuration, event monitoring workflows, and documented C-only native-pointer exclusions.
...
- [ ] Complete OPC UA Lua client async, subscription, browse callback, method-call, remote node-management, and PubSub-related workflows with explicit Lua callback lifetime/error semantics and deterministic local e2e.
