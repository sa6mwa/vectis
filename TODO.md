...
- [ ] Complete the dependency-native OPC UA Lua server facade over all remaining relevant `cpkt_opcua_server_*` APIs, including security config, access-control callbacks, PubSub/MQTT configuration, event monitoring workflows, async-safe callback queueing, and documented C-only native-pointer exclusions.
...
- [ ] Complete OPC UA Lua client async browse, async node-creation, native-pointer exclusion, and PubSub-related workflows with explicit Lua callback lifetime/error semantics and deterministic local e2e.
