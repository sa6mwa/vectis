...
- [ ] Complete the dependency-native OPC UA Lua server facade over all remaining relevant `cpkt_opcua_server_*` APIs, including security config, live PubSub/MQTT configuration validation, event monitoring workflows, async-safe callback queueing, and documented C-only native-pointer exclusions.
