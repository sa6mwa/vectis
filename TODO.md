...
- [ ] Complete the dependency-native OPC UA Lua server facade over all relevant `cpkt_opcua_server_*` APIs, including security config, access-control callbacks, object/variable/reference/data/view type creation, methods, browse/path traversal, references, scalar and array attributes, events, PubSub/MQTT configuration, and documented C-only native-pointer exclusions.
