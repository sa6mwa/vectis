...
- [ ] Complete the dependency-native OPC UA Lua server facade over all remaining relevant `cpkt_opcua_server_*` APIs, including live PubSub/MQTT configuration validation, event monitoring workflows, and async-safe callback queueing.
...
- [ ] Complete OPC UA Lua client PubSub-related workflows with explicit Lua callback lifetime/error semantics and deterministic local e2e.
