...
vectis_webdav_path_in_root(const char *root, const char *path,
  ...
  if (normalized[1] == '\0') {
    ...
    written =
        snprintf(out, VECTIS_WEBDAV_STORAGE_PATH_MAX, "%s", normalized);
  } else {
    written =
...
static int vectis_webdav_usage(const vectis_webdav_config *config,
  ...
  *resources = 0u;
  if (vectis_webdav_direct_root(config)) {
    return vectis_webdav_usage_path(content, total, resources);
  }
  if (!vectis_webdav_disk_path(config, "tombstones", "/", tombstones)) {
    return 0;
  }
  return vectis_webdav_usage_path(content, total, resources) &&
         vectis_webdav_usage_path(tombstones, total, resources);
}
...
vectis_webdav_status vectis_webdav_read(const vectis_webdav_config *config,
  ...
  *body_size = 0u;
  ...
  if (!vectis_webdav_read_file(entry->storage_path, body, body_size,
                               entry->etag)) {
    vectis_webdav_unlock(lock_fd);
    return VECTIS_WEBDAV_IO;
  }
...
vectis_webdav_store_locked(const vectis_webdav_config *config,
  ...
  if (!vectis_webdav_write_atomic(disk, body, body_size) ||
...
static int vectis_webdav_disk_usage(const char *path, uint64_t *total,
  ...
  if (S_ISDIR(st.st_mode)) {
    ...
    (*resources)++;
    return vectis_webdav_usage_path(path, total, resources);
  }
  return 0;
}
...
vectis_webdav_status vectis_webdav_delete(const vectis_webdav_config *config,
  ...
  if (vectis_webdav_direct_root(config)) {
    (void)usage;
    (void)resources;
    if (!vectis_webdav_remove_tree(content)) {
      vectis_webdav_unlock(lock_fd);
      return VECTIS_WEBDAV_IO;
    }
    vectis_webdav_unlock(lock_fd);
...
static vectis_webdav_status vectis_webdav_copy_or_move_direct(
  ...
  if (remove_source && !vectis_webdav_remove_tree(source_disk)) {
    status = VECTIS_WEBDAV_IO;
    goto rollback_destination;
  }
