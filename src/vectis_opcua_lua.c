...
#define VECTIS_OPCUA_EXPANDED_NODE_ID "opcua.expanded_node_id"
...
typedef struct vectis_opcua_lua_expanded_node_id {
  cpkt_opcua_expanded_node_id node_id;
  size_t storage_size;
  unsigned char storage[1];
} vectis_opcua_lua_expanded_node_id;
...
/* Keep the owner as a userdata edge rather than a registry root: Lua can then
 * collect an otherwise unreachable owner/handle cycle. */
static void vectis_opcua_lua_retain_owner(lua_State *lua, int index,
                                          void *key) {
  index = lua_absindex(lua, index);
  lua_getiuservalue(lua, index, 1);
...
static void vectis_opcua_lua_release_owners(lua_State *lua, int index) {
  index = lua_absindex(lua, index);
  lua_pushnil(lua);
  lua_setiuservalue(lua, index, 1);
}
...
static size_t
vectis_opcua_lua_node_id_storage_size(const cpkt_opcua_node_id *node_id) {
  if (node_id->identifier_type == CPKT_OPCUA_NODE_ID_STRING) {
    return strlen(node_id->string) + 1u;
  }
  if (node_id->identifier_type == CPKT_OPCUA_NODE_ID_BYTE_STRING) {
    return node_id->byte_string_length;
  }
  return 0u;
}

static unsigned char *
vectis_opcua_lua_copy_node_id_storage(cpkt_opcua_node_id *target,
  ...
  *target = *source;
  if (source->identifier_type == CPKT_OPCUA_NODE_ID_STRING) {
    size = strlen(source->string) + 1u;
    memcpy(storage, source->string, size);
    target->string = (const char *)storage;
    return storage + size;
  }
  if (source->identifier_type == CPKT_OPCUA_NODE_ID_BYTE_STRING) {
    size = source->byte_string_length;
    memcpy(storage, source->byte_string, size);
    target->byte_string = storage;
    return storage + size;
  }
  return storage;
}

static vectis_opcua_lua_expanded_node_id *
vectis_opcua_lua_new_expanded_node_id(lua_State *lua, size_t storage_size) {
  vectis_opcua_lua_expanded_node_id *node;
  size_t alloc_size;

  alloc_size = offsetof(vectis_opcua_lua_expanded_node_id, storage);
  if (storage_size > ((size_t)-1) - alloc_size) {
    luaL_error(lua, "opcua expanded node id allocation size overflow");
    return NULL;
  }
  if (storage_size == 0u) {
    storage_size = 1u;
  }
  node = (vectis_opcua_lua_expanded_node_id *)lua_newuserdatauv(
      lua, alloc_size + storage_size, 0);
  node->node_id = cpkt_opcua_expanded_node_id_local(cpkt_opcua_node_id_null());
  node->storage_size = storage_size;
  memset(node->storage, 0, storage_size);
  luaL_getmetatable(lua, VECTIS_OPCUA_EXPANDED_NODE_ID);
  lua_setmetatable(lua, -2);
  return node;
}

static vectis_opcua_lua_expanded_node_id *
vectis_opcua_lua_check_expanded_node_id(lua_State *lua, int index) {
  return (vectis_opcua_lua_expanded_node_id *)luaL_checkudata(
      lua, index, VECTIS_OPCUA_EXPANDED_NODE_ID);
}

static cpkt_opcua_expanded_node_id
vectis_opcua_lua_expanded_node_id_at(lua_State *lua, int index) {
  if (luaL_testudata(lua, index, VECTIS_OPCUA_EXPANDED_NODE_ID) != NULL) {
    return vectis_opcua_lua_check_expanded_node_id(lua, index)->node_id;
  }
  if (luaL_testudata(lua, index, VECTIS_OPCUA_NODE_ID) != NULL) {
    return cpkt_opcua_expanded_node_id_local(
        vectis_opcua_lua_check_node_id(lua, index)->node_id);
  }
  (void)luaL_error(lua, "opcua expanded node id expected");
  return cpkt_opcua_expanded_node_id_local(cpkt_opcua_node_id_null());
}

static cpkt_opcua_expanded_node_id vectis_opcua_lua_expanded_node_id_field(
    lua_State *lua, int index, const char *field, const char *context) {
  cpkt_opcua_expanded_node_id node_id;
  ...
  node_id = vectis_opcua_lua_expanded_node_id_at(lua, -1);
  lua_pop(lua, 1);
  return node_id;
}

static int vectis_opcua_lua_push_expanded_node_id_copy(
    lua_State *lua, const cpkt_opcua_expanded_node_id *input) {
  vectis_opcua_lua_expanded_node_id *node;
  unsigned char *cursor;
  size_t storage_size;
  size_t uri_size;

  storage_size = vectis_opcua_lua_node_id_storage_size(&input->node_id);
  uri_size =
      input->namespace_uri != NULL ? input->namespace_uri_length + 1u : 0u;
  storage_size = vectis_opcua_lua_checked_size_add(lua, storage_size, uri_size,
                                                   "opcua expanded node id");
  node = vectis_opcua_lua_new_expanded_node_id(lua, storage_size);
  node->node_id = *input;
  cursor = node->storage;
  cursor = vectis_opcua_lua_copy_node_id_storage(&node->node_id.node_id,
                                                 &input->node_id, cursor);
  if (input->namespace_uri != NULL) {
    memcpy(cursor, input->namespace_uri, input->namespace_uri_length);
    cursor[input->namespace_uri_length] = '\0';
    node->node_id.namespace_uri = (const char *)cursor;
  }
  return 1;
}

static int vectis_opcua_lua_expanded_node_id_print_to_lua(
    lua_State *lua, cpkt_opcua_expanded_node_id node_id) {
  char stack_buffer[128];
  char *buffer;
  size_t required_size;
  cpkt_opcua_result result;
  ...
  buffer = stack_buffer;
  required_size = 0u;
  result = cpkt_opcua_expanded_node_id_print(
      node_id, buffer, sizeof(stack_buffer), &required_size);
  if (result == CPKT_OPCUA_ERR_RANGE && required_size > sizeof(stack_buffer)) {
    buffer = (char *)malloc(required_size + 1u);
    if (buffer == NULL) {
      return luaL_error(lua, "opcua expanded node id print allocation failed");
    }
    result = cpkt_opcua_expanded_node_id_print(node_id, buffer,
                                               required_size + 1u, NULL);
  }
  if (result != CPKT_OPCUA_OK) {
    if (buffer != stack_buffer) {
      free(buffer);
...
static int vectis_opcua_lua_expanded_node_id_tostring(lua_State *lua) {
  vectis_opcua_lua_expanded_node_id *node;

  node = vectis_opcua_lua_check_expanded_node_id(lua, 1);
  return vectis_opcua_lua_expanded_node_id_print_to_lua(lua, node->node_id);
}

static int vectis_opcua_lua_expanded_node_id_eq(lua_State *lua) {
  vectis_opcua_lua_expanded_node_id *left;
  vectis_opcua_lua_expanded_node_id *right;

  left = vectis_opcua_lua_check_expanded_node_id(lua, 1);
  right = vectis_opcua_lua_check_expanded_node_id(lua, 2);
  lua_pushboolean(
      lua, cpkt_opcua_expanded_node_id_equal(left->node_id, right->node_id));
  return 1;
}

static int vectis_opcua_lua_expanded_node_id_node(lua_State *lua) {
  vectis_opcua_lua_expanded_node_id *node;

  node = vectis_opcua_lua_check_expanded_node_id(lua, 1);
  return vectis_opcua_lua_push_node_id_copy(lua, &node->node_id.node_id);
}

static int vectis_opcua_lua_expanded_node_id_namespace_uri(lua_State *lua) {
  vectis_opcua_lua_expanded_node_id *node;

  node = vectis_opcua_lua_check_expanded_node_id(lua, 1);
  if (node->node_id.namespace_uri == NULL) {
    lua_pushnil(lua);
  } else {
    lua_pushlstring(lua, node->node_id.namespace_uri,
                    node->node_id.namespace_uri_length);
  }
  return 1;
}

static int vectis_opcua_lua_expanded_node_id_server_index(lua_State *lua) {
  vectis_opcua_lua_expanded_node_id *node;

  node = vectis_opcua_lua_check_expanded_node_id(lua, 1);
  lua_pushinteger(lua, (lua_Integer)node->node_id.server_index);
  return 1;
}

static int vectis_opcua_lua_expanded_node_id_parse(lua_State *lua) {
  const char *text;
  cpkt_opcua_result result;
  ...
  buffer = stack_buffer;
  required_size = 0u;
  result = cpkt_opcua_expanded_node_id_parse(
      text, &parsed, buffer, sizeof(stack_buffer), &required_size);
  if (result == CPKT_OPCUA_ERR_RANGE && required_size > sizeof(stack_buffer)) {
    buffer = (char *)malloc(required_size + 1u);
    if (buffer == NULL) {
      return luaL_error(lua, "opcua expanded node_id parse allocation failed");
    }
    result = cpkt_opcua_expanded_node_id_parse(text, &parsed, buffer,
                                               required_size + 1u, NULL);
  }
  if (result != CPKT_OPCUA_OK) {
    if (buffer != stack_buffer) {
    ...
    return vectis_opcua_lua_push_error(lua, result, 0u,
                                       "opcua expanded_node_id parse");
  }
  (void)vectis_opcua_lua_push_expanded_node_id_copy(lua, &parsed);
  if (buffer != stack_buffer) {
    free(buffer);
  }
  return 1;
...
static int vectis_opcua_lua_expanded_node_id_local_lua(lua_State *lua) {
  cpkt_opcua_node_id node_id;
  ...
  return vectis_opcua_lua_push_expanded_node_id_copy(lua, &expanded);
}

static int vectis_opcua_lua_expanded_node_id_uri_lua(lua_State *lua) {
  const char *namespace_uri;
  size_t namespace_uri_length;
  cpkt_opcua_node_id node_id;
  cpkt_opcua_expanded_node_id expanded;

  namespace_uri = luaL_checklstring(lua, 1, &namespace_uri_length);
...
static int vectis_opcua_lua_expanded_node_id_server_lua(lua_State *lua) {
  unsigned long server_index;
  cpkt_opcua_node_id node_id;
  cpkt_opcua_expanded_node_id expanded;

  server_index = vectis_opcua_lua_check_ulong(lua, 1, "server_index");
  node_id = vectis_opcua_lua_node_id_at(lua, 2);
  expanded = cpkt_opcua_expanded_node_id_server(server_index, node_id);
  return vectis_opcua_lua_push_expanded_node_id_copy(lua, &expanded);
}

static int vectis_opcua_lua_expanded_node_id_server_uri_lua(lua_State *lua) {
  unsigned long server_index;
  const char *namespace_uri;
  size_t namespace_uri_length;
  cpkt_opcua_node_id node_id;
  ...
  server_index = vectis_opcua_lua_check_ulong(lua, 1, "server_index");
  namespace_uri = luaL_checklstring(lua, 2, &namespace_uri_length);
  node_id = vectis_opcua_lua_node_id_at(lua, 3);
  expanded = cpkt_opcua_expanded_node_id_server_uri(
      server_index, namespace_uri, namespace_uri_length, node_id);
  return vectis_opcua_lua_push_expanded_node_id_copy(lua, &expanded);
}
...
static void vectis_opcua_lua_client_free_monitor_callbacks(
    vectis_opcua_lua_client *client) {
  vectis_opcua_lua_monitor_callback *callback;
  vectis_opcua_lua_monitor_callback *next;

  callback = client->monitors;
...
static vectis_opcua_lua_monitor_callback *vectis_opcua_lua_monitor_callback_new(
    lua_State *lua, vectis_opcua_lua_client *client, int callback_index,
    cpkt_opcua_subscription_id subscription_id) {
  vectis_opcua_lua_monitor_callback *callback;

  callback = (vectis_opcua_lua_monitor_callback *)calloc(1u, sizeof(*callback));
  if (callback == NULL) {
    (void)luaL_error(lua, "opcua monitor callback allocation failed");
    return NULL;
  }
  callback->owner = lua;
...
static void vectis_opcua_lua_client_keep_monitor_callback(
    vectis_opcua_lua_client *client,
    vectis_opcua_lua_monitor_callback *callback) {
  callback->next = client->monitors;
  client->monitors = callback;
}

static void vectis_opcua_lua_client_forget_monitor_callback(
    vectis_opcua_lua_client *client, cpkt_opcua_subscription_id subscription_id,
    cpkt_opcua_monitored_item_id monitored_item_id) {
  vectis_opcua_lua_monitor_callback *callback;
  vectis_opcua_lua_monitor_callback *previous;

  previous = NULL;
  callback = client->monitors;
  while (callback != NULL) {
    if (callback->subscription_id == subscription_id &&
        callback->monitored_item_id == monitored_item_id) {
      if (previous == NULL) {
        client->monitors = callback->next;
      } else {
...
static void vectis_opcua_lua_client_forget_subscription_callbacks(
    vectis_opcua_lua_client *client,
    cpkt_opcua_subscription_id subscription_id) {
  vectis_opcua_lua_monitor_callback *callback;
  vectis_opcua_lua_monitor_callback *previous;
...
static void vectis_opcua_lua_async_callback_free(
  ...
  if (callback->owner != NULL && callback->browse_entries_ref != LUA_NOREF) {
    luaL_unref(callback->owner, LUA_REGISTRYINDEX,
...
static void vectis_opcua_lua_async_callback_prepare_browse_entries(
    lua_State *lua, vectis_opcua_lua_async_callback *callback) {
  lua_newtable(lua);
  callback->browse_entries_ref = luaL_ref(lua, LUA_REGISTRYINDEX);
  callback->browse_entry_count = 0u;
}
...
vectis_opcua_lua_client_set_callback_error(vectis_opcua_lua_client *client,
  ...
  lua = client->owner;
  lua_pushstring(lua, message != NULL ? message : "opcua callback failed");
  client->callback_error_ref = luaL_ref(lua, LUA_REGISTRYINDEX);
}

static int vectis_opcua_lua_client_take_callback_error(
    lua_State *lua, vectis_opcua_lua_client *client, const char *context) {
  const char *message;

  if (client->callback_error_ref == LUA_NOREF) {
    return 0;
  }
  lua_rawgeti(lua, LUA_REGISTRYINDEX, client->callback_error_ref);
  luaL_unref(lua, LUA_REGISTRYINDEX, client->callback_error_ref);
  client->callback_error_ref = LUA_NOREF;
  ...
  lua_setfield(lua, -2, "status");
  vectis_opcua_lua_set_field_string(lua, "status_string", "state");
  lua_pushinteger(lua, VECTIS_ERROR_SOURCE_CPKT);
  lua_setfield(lua, -2, "source_code");
  vectis_opcua_lua_set_field_string(lua, "source", "cpkt");
  vectis_opcua_lua_set_field_string(lua, "dependency", "opcua");
  lua_pushinteger(lua, (lua_Integer)CPKT_OPCUA_ERR_CALLBACK);
  lua_setfield(lua, -2, "dependency_code");
  lua_pushinteger(lua, (lua_Integer)CPKT_OPCUA_ERR_CALLBACK);
  lua_setfield(lua, -2, "result");
  vectis_opcua_lua_set_field_string(
      lua, "result_string", cpkt_opcua_result_string(CPKT_OPCUA_ERR_CALLBACK));
  lua_pushinteger(lua, 0);
  lua_setfield(lua, -2, "opcua_status");
  vectis_opcua_lua_set_field_string(lua, "opcua_status_name",
                                    cpkt_opcua_status_name(0u));
  lua_pushfstring(lua, "%s: %s", context,
                  message != NULL ? message : "opcua callback failed");
  lua_setfield(lua, -2, "message");
  lua_remove(lua, -3);
  return 2;
}
...
static void
vectis_opcua_lua_push_browse_entry_table(lua_State *lua,
                                         const cpkt_opcua_browse_entry *entry) {
  lua_newtable(lua);
...
static int
vectis_opcua_lua_async_browse_entry_cb(const cpkt_opcua_browse_entry *entry,
                                       void *user) {
  vectis_opcua_lua_async_callback *callback;
  lua_State *lua;
  ...
  lua = callback->owner;
  lua_rawgeti(lua, LUA_REGISTRYINDEX, callback->browse_entries_ref);
  vectis_opcua_lua_push_browse_entry_table(lua, entry);
  ++callback->browse_entry_count;
  lua_rawseti(lua, -2, (lua_Integer)callback->browse_entry_count);
  lua_pop(lua, 1);
  return 0;
}

static void vectis_opcua_lua_async_browse_cb(cpkt_opcua_request_id request_id,
                                             cpkt_opcua_result result,
                                             cpkt_opcua_status status,
                                             void *user) {
  vectis_opcua_lua_async_callback *callback;
  ...
  lua_rawgeti(lua, LUA_REGISTRYINDEX, callback->callback_ref);
  vectis_opcua_lua_push_async_result_base(lua, request_id, result, status);
  if (callback->browse_entries_ref != LUA_NOREF) {
    lua_rawgeti(lua, LUA_REGISTRYINDEX, callback->browse_entries_ref);
    lua_setfield(lua, -2, "entries");
  }
  lua_pushinteger(lua, (lua_Integer)callback->browse_entry_count);
  lua_setfield(lua, -2, "entry_count");
  if (lua_pcall(lua, 1, 0, 0) != LUA_OK) {
    vectis_opcua_lua_client_set_callback_error(callback->client,
                                               lua_tostring(lua, -1));
    lua_pop(lua, 1);
...
static void vectis_opcua_lua_async_node_cb(cpkt_opcua_request_id request_id,
                                           cpkt_opcua_result result,
  ...
  if (result == CPKT_OPCUA_OK && node_id != NULL) {
    ...
    lua_setfield(lua, -2, "node_id");
  }
  if (callback->required_node_id_size > callback->node_id_buffer_size) {
    lua_pushinteger(lua, (lua_Integer)callback->required_node_id_size);
    lua_setfield(lua, -2, "required_node_id_size");
  }
  if (lua_pcall(lua, 1, 0, 0) != LUA_OK) {
    vectis_opcua_lua_client_set_callback_error(callback->client,
                                               lua_tostring(lua, -1));
...
static void
vectis_opcua_lua_data_change_cb(cpkt_opcua_subscription_id subscription_id,
                                cpkt_opcua_monitored_item_id monitored_item_id,
                                const cpkt_opcua_value *value,
...
static int vectis_opcua_lua_client_close(lua_State *lua) {
  ...
  vectis_opcua_lua_release_owners(lua, 1);
  client->owner = NULL;
...
static int vectis_opcua_lua_client_new(lua_State *lua) {
  ...
  client = (vectis_opcua_lua_client *)lua_newuserdatauv(
      lua, sizeof(vectis_opcua_lua_client), 1);
  client->client = NULL;
  client->owner = lua;
  client->callback_error_ref = LUA_NOREF;
  client->monitors = NULL;
  ...
  vectis_opcua_lua_retain_owner(lua, -1, client);
...
static int vectis_opcua_lua_client_iterate(lua_State *lua) {
  ...
  callback_result_count = vectis_opcua_lua_client_take_callback_error(
      lua, client_ud, "opcua client iterate");
  if (callback_result_count != 0) {
    return callback_result_count;
  }
  lua_pushboolean(lua, 1);
  return 1;
}

static int vectis_opcua_lua_client_create_subscription(lua_State *lua) {
  cpkt_opcua_client *client;
  ...
  client = vectis_opcua_lua_client_handle(lua, 1);
  publishing_interval_ms = luaL_checknumber(lua, 2);
  subscription_id = 0u;
  status = 0u;
  result = cpkt_opcua_client_create_subscription(client, publishing_interval_ms,
                                                 &subscription_id, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client create subscription");
  }
  lua_pushinteger(lua, (lua_Integer)subscription_id);
  return 1;
}

static int vectis_opcua_lua_client_modify_subscription(lua_State *lua) {
  cpkt_opcua_client *client;
  cpkt_opcua_subscription_id subscription_id;
  double publishing_interval_ms;
  cpkt_opcua_status status;
  ...
  publishing_interval_ms = luaL_checknumber(lua, 3);
  status = 0u;
  result = cpkt_opcua_client_modify_subscription(
      client, subscription_id, publishing_interval_ms, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client modify subscription");
  }
  lua_pushboolean(lua, 1);
  return 1;
}

static int vectis_opcua_lua_client_delete_subscription(lua_State *lua) {
  vectis_opcua_lua_client *client_ud;
  cpkt_opcua_client *client;
  cpkt_opcua_subscription_id subscription_id;
  cpkt_opcua_status status;
  ...
  status = 0u;
  result =
      cpkt_opcua_client_delete_subscription(client, subscription_id, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client delete subscription");
  }
  vectis_opcua_lua_client_forget_subscription_callbacks(client_ud,
                                                        subscription_id);
  lua_pushboolean(lua, 1);
  return 1;
}

static void vectis_opcua_lua_monitor_options_from_table(
    lua_State *lua, int index, cpkt_opcua_monitor_options *options) {
  cpkt_opcua_monitor_options_default(options);
  luaL_checktype(lua, index, LUA_TTABLE);
  lua_getfield(lua, index, "sampling_interval_ms");
  if (!lua_isnil(lua, -1)) {
    options->sampling_interval_ms = luaL_checknumber(lua, -1);
  }
  lua_pop(lua, 1);
  options->queue_size = vectis_opcua_lua_table_ulong(lua, index, "queue_size",
                                                     options->queue_size);
  options->discard_oldest = vectis_opcua_lua_table_bool(
      lua, index, "discard_oldest", options->discard_oldest);
  options->deadband_type = vectis_opcua_lua_table_int(
      lua, index, "deadband_type", options->deadband_type);
  lua_getfield(lua, index, "deadband_value");
  if (!lua_isnil(lua, -1)) {
    options->deadband_value = luaL_checknumber(lua, -1);
  }
  lua_pop(lua, 1);
}

static int vectis_opcua_lua_client_monitor_value(lua_State *lua) {
  vectis_opcua_lua_client *client_ud;
  cpkt_opcua_client *client;
  cpkt_opcua_subscription_id subscription_id;
  cpkt_opcua_node_id node_id;
  ...
  status = 0u;
  result = cpkt_opcua_client_monitor_value(
      client, subscription_id, node_id, sampling_interval_ms,
      vectis_opcua_lua_data_change_cb, callback, &monitored_item_id, &status);
  if (result != CPKT_OPCUA_OK) {
    vectis_opcua_lua_monitor_callback_free(callback);
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client monitor value");
  }
  callback->monitored_item_id = monitored_item_id;
...
static int vectis_opcua_lua_client_monitor_value_ex(lua_State *lua) {
  vectis_opcua_lua_client *client_ud;
  cpkt_opcua_client *client;
  cpkt_opcua_subscription_id subscription_id;
  cpkt_opcua_node_id node_id;
  ...
  subscription_id = vectis_opcua_lua_check_ulong(lua, 2, "subscription_id");
  node_id = vectis_opcua_lua_node_id_at(lua, 3);
  vectis_opcua_lua_monitor_options_from_table(lua, 4, &options);
  luaL_checktype(lua, 5, LUA_TFUNCTION);
  callback =
      vectis_opcua_lua_monitor_callback_new(lua, client_ud, 5, subscription_id);
  monitored_item_id = 0u;
  status = 0u;
  result = cpkt_opcua_client_monitor_value_ex(
      client, subscription_id, node_id, &options,
      vectis_opcua_lua_data_change_cb, callback, &monitored_item_id, &status);
  if (result != CPKT_OPCUA_OK) {
    vectis_opcua_lua_monitor_callback_free(callback);
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client monitor value ex");
  }
  callback->monitored_item_id = monitored_item_id;
  vectis_opcua_lua_client_keep_monitor_callback(client_ud, callback);
  lua_pushinteger(lua, (lua_Integer)monitored_item_id);
...
static int vectis_opcua_lua_client_set_monitoring_mode(lua_State *lua) {
  cpkt_opcua_client *client;
  cpkt_opcua_subscription_id subscription_id;
  cpkt_opcua_monitored_item_id monitored_item_id;
  int monitoring_mode;
  ...
  client = vectis_opcua_lua_client_handle(lua, 1);
  subscription_id = vectis_opcua_lua_check_ulong(lua, 2, "subscription_id");
  monitored_item_id = vectis_opcua_lua_check_ulong(lua, 3, "monitored_item_id");
  monitoring_mode = (int)luaL_checkinteger(lua, 4);
  status = 0u;
  result = cpkt_opcua_client_set_monitoring_mode(
      client, subscription_id, monitored_item_id, monitoring_mode, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client set monitoring mode");
  }
  lua_pushboolean(lua, 1);
  return 1;
}

static int vectis_opcua_lua_client_delete_monitored_item(lua_State *lua) {
  vectis_opcua_lua_client *client_ud;
  cpkt_opcua_client *client;
  cpkt_opcua_subscription_id subscription_id;
  cpkt_opcua_monitored_item_id monitored_item_id;
  ...
  monitored_item_id = vectis_opcua_lua_check_ulong(lua, 3, "monitored_item_id");
  status = 0u;
  result = cpkt_opcua_client_delete_monitored_item(client, subscription_id,
                                                   monitored_item_id, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client delete monitored item");
  }
...
static size_t vectis_opcua_lua_async_buffer_size(lua_State *lua, int index,
  ...
  size = vectis_opcua_lua_table_ulong(lua, index, "string_buffer_size",
                                      default_size);
  size = vectis_opcua_lua_table_ulong(lua, index, "node_id_buffer_size", size);
...
static int vectis_opcua_lua_client_browse_children_async(lua_State *lua) {
  vectis_opcua_lua_client *client_ud;
  cpkt_opcua_client *client;
  ...
  luaL_checktype(lua, 3, LUA_TFUNCTION);
  vectis_opcua_lua_browse_options_from_lua(lua, 4, &options);
  callback = vectis_opcua_lua_async_callback_new(lua, client_ud, 3);
  vectis_opcua_lua_async_callback_prepare_browse_entries(lua, callback);
  request_id = 0u;
  status = 0u;
  result = cpkt_opcua_client_browse_children_async(
      client, parent_node_id, &options, vectis_opcua_lua_async_browse_entry_cb,
      vectis_opcua_lua_async_browse_cb, callback, &request_id, &status);
  if (result != CPKT_OPCUA_OK) {
    vectis_opcua_lua_async_callback_free(callback);
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client browse children async");
  }
  callback->request_id = request_id;
  vectis_opcua_lua_client_keep_async_callback(client_ud, callback);
  lua_pushinteger(lua, (lua_Integer)request_id);
...
static int vectis_opcua_lua_client_add_object_async(lua_State *lua) {
  vectis_opcua_lua_client *client_ud;
  cpkt_opcua_client *client;
  cpkt_opcua_node_id node_id;
  cpkt_opcua_node_id parent_node_id;
  ...
  node_id = vectis_opcua_lua_node_id_field(lua, 2, "node_id",
                                           "opcua client add_object_async");
  parent_node_id = vectis_opcua_lua_node_id_field(
      lua, 2, "parent_node_id", "opcua client add_object_async");
  ...
  buffer_size = vectis_opcua_lua_async_buffer_size(
      lua, 4, 512u, "opcua client add_object_async");
  callback = vectis_opcua_lua_async_callback_new(lua, client_ud, 3);
  callback->node_id_buffer = (char *)malloc(buffer_size);
  if (callback->node_id_buffer == NULL) {
    vectis_opcua_lua_async_callback_free(callback);
    return luaL_error(lua, "opcua add_object_async buffer allocation failed");
  }
  callback->node_id_buffer_size = buffer_size;
  callback->required_node_id_size = 0u;
  request_id = 0u;
  status = 0u;
  result = cpkt_opcua_client_add_object_async(
      client, node_id, parent_node_id, browse_name, display_name,
      vectis_opcua_lua_async_node_cb, callback, &request_id, &callback->node_id,
      callback->node_id_buffer, callback->node_id_buffer_size,
      &callback->required_node_id_size, &status);
  if (result != CPKT_OPCUA_OK) {
    vectis_opcua_lua_async_callback_free(callback);
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client add object async");
  }
  callback->request_id = request_id;
  vectis_opcua_lua_client_keep_async_callback(client_ud, callback);
  lua_pushinteger(lua, (lua_Integer)request_id);
...
static int vectis_opcua_lua_client_add_variable_async(lua_State *lua) {
  vectis_opcua_lua_client *client_ud;
  cpkt_opcua_client *client;
  cpkt_opcua_node_id node_id;
  cpkt_opcua_node_id parent_node_id;
  ...
  luaL_checktype(lua, 3, LUA_TFUNCTION);
  node_id = vectis_opcua_lua_node_id_field(lua, 2, "node_id",
                                           "opcua client add_variable_async");
  parent_node_id = vectis_opcua_lua_node_id_field(
      lua, 2, "parent_node_id", "opcua client add_variable_async");
  browse_name = vectis_opcua_lua_table_string(lua, 2, "browse_name");
  display_name = vectis_opcua_lua_table_string(lua, 2, "display_name");
  vectis_opcua_lua_value_field(lua, 2, "value",
                               "opcua client add_variable_async", &value);
  buffer_size = vectis_opcua_lua_async_buffer_size(
      lua, 4, 512u, "opcua client add_variable_async");
  callback = vectis_opcua_lua_async_callback_new(lua, client_ud, 3);
  callback->node_id_buffer = (char *)malloc(buffer_size);
  if (callback->node_id_buffer == NULL) {
    vectis_opcua_lua_async_callback_free(callback);
    return luaL_error(lua, "opcua add_variable_async buffer allocation failed");
  }
  callback->node_id_buffer_size = buffer_size;
  callback->required_node_id_size = 0u;
  request_id = 0u;
  status = 0u;
  result = cpkt_opcua_client_add_variable_async(
  ...
  if (result != CPKT_OPCUA_OK) {
    vectis_opcua_lua_async_callback_free(callback);
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client add variable async");
  }
  callback->request_id = request_id;
  vectis_opcua_lua_client_keep_async_callback(client_ud, callback);
  lua_pushinteger(lua, (lua_Integer)request_id);
...
static int vectis_opcua_lua_client_endpoint_count(lua_State *lua) {
  cpkt_opcua_client *client;
  ...
  client = vectis_opcua_lua_client_handle(lua, 1);
  server_url = luaL_checkstring(lua, 2);
  count = 0u;
  status = 0u;
  result =
      cpkt_opcua_client_get_endpoint_count(client, server_url, &count, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua endpoint count");
  }
  lua_pushinteger(lua, (lua_Integer)count);
  return 1;
}

static int vectis_opcua_lua_client_endpoint_url_at(lua_State *lua) {
  cpkt_opcua_client *client;
  const char *server_url;
  lua_Integer endpoint_index_value;
  ...
  client = vectis_opcua_lua_client_handle(lua, 1);
  server_url = luaL_checkstring(lua, 2);
  endpoint_index_value = luaL_checkinteger(lua, 3);
  if (endpoint_index_value <= 0) {
    return luaL_error(lua, "endpoint_index is 1-based");
  }
  endpoint_index = (size_t)(endpoint_index_value - 1);
  buffer = stack_buffer;
  required_size = 0u;
  status = 0u;
  result = cpkt_opcua_client_get_endpoint_url(
      client, server_url, endpoint_index, buffer, sizeof(stack_buffer),
      &required_size, &status);
  if (result == CPKT_OPCUA_ERR_RANGE && required_size > sizeof(stack_buffer)) {
    buffer = (char *)malloc(required_size + 1u);
    if (buffer == NULL) {
      return luaL_error(lua, "opcua endpoint url allocation failed");
    }
    result = cpkt_opcua_client_get_endpoint_url(
        client, server_url, endpoint_index, buffer, required_size + 1u, NULL,
        &status);
  }
  if (result != CPKT_OPCUA_OK) {
    if (buffer != stack_buffer) {
...
static int vectis_opcua_lua_client_endpoints(lua_State *lua) {
  cpkt_opcua_client *client;
  const char *server_url;
  size_t count;
  size_t i;
  ...
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua endpoints count");
  }
  lua_newtable(lua);
  for (i = 0u; i < count; ++i) {
    buffer = stack_buffer;
    required_size = 0u;
    result = cpkt_opcua_client_get_endpoint_url(client, server_url, i, buffer,
                                                sizeof(stack_buffer),
                                                &required_size, &status);
    if (result == CPKT_OPCUA_ERR_RANGE &&
        required_size > sizeof(stack_buffer)) {
      buffer = (char *)malloc(required_size + 1u);
      if (buffer == NULL) {
        return luaL_error(lua, "opcua endpoint url allocation failed");
      }
      result = cpkt_opcua_client_get_endpoint_url(
          client, server_url, i, buffer, required_size + 1u, NULL, &status);
    }
    if (result != CPKT_OPCUA_OK) {
      if (buffer != stack_buffer) {
        free(buffer);
...
static int vectis_opcua_lua_client_server_count(lua_State *lua) {
  cpkt_opcua_client *client;
  const char *server_url;
  size_t count;
  cpkt_opcua_status status;
  ...
  status = 0u;
  result =
      cpkt_opcua_client_find_server_count(client, server_url, &count, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua server count");
  }
...
static int vectis_opcua_lua_client_server_string_at(
    lua_State *lua,
    cpkt_opcua_result (*fn)(cpkt_opcua_client *, const char *, size_t, char *,
                            size_t, size_t *, cpkt_opcua_status *),
    const char *context) {
  cpkt_opcua_client *client;
  const char *server_url;
  lua_Integer server_index_value;
  size_t server_index;
  char stack_buffer[512];
  char *buffer;
  size_t required_size;
  ...
  client = vectis_opcua_lua_client_handle(lua, 1);
  server_url = luaL_checkstring(lua, 2);
  server_index_value = luaL_checkinteger(lua, 3);
  if (server_index_value <= 0) {
    return luaL_error(lua, "server_index is 1-based");
  }
  server_index = (size_t)(server_index_value - 1);
  buffer = stack_buffer;
  required_size = 0u;
  status = 0u;
  result = fn(client, server_url, server_index, buffer, sizeof(stack_buffer),
              &required_size, &status);
  if (result == CPKT_OPCUA_ERR_RANGE && required_size > sizeof(stack_buffer)) {
    buffer = (char *)malloc(required_size + 1u);
    if (buffer == NULL) {
      return luaL_error(lua, "opcua server discovery string allocation failed");
    }
    result = fn(client, server_url, server_index, buffer, required_size + 1u,
                NULL, &status);
  }
  if (result != CPKT_OPCUA_OK) {
    if (buffer != stack_buffer) {
...
static int vectis_opcua_lua_client_server_application_uri(lua_State *lua) {
  return vectis_opcua_lua_client_server_string_at(
      lua, cpkt_opcua_client_find_server_application_uri,
      "opcua server application uri");
}

static int vectis_opcua_lua_client_server_application_name(lua_State *lua) {
  return vectis_opcua_lua_client_server_string_at(
      lua, cpkt_opcua_client_find_server_application_name,
      "opcua server application name");
}

static int vectis_opcua_lua_client_find_servers(lua_State *lua) {
  cpkt_opcua_client *client;
  const char *server_url;
  size_t count;
  size_t i;
  char uri_stack_buffer[512];
  char name_stack_buffer[512];
  char *uri_buffer;
  char *name_buffer;
  size_t uri_required_size;
  ...
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua find servers count");
  }
  lua_newtable(lua);
  for (i = 0u; i < count; ++i) {
    uri_buffer = uri_stack_buffer;
    uri_required_size = 0u;
    result = cpkt_opcua_client_find_server_application_uri(
        client, server_url, i, uri_buffer, sizeof(uri_stack_buffer),
        &uri_required_size, &status);
    if (result == CPKT_OPCUA_ERR_RANGE &&
        uri_required_size > sizeof(uri_stack_buffer)) {
      uri_buffer = (char *)malloc(uri_required_size + 1u);
      if (uri_buffer == NULL) {
        return luaL_error(lua, "opcua server uri allocation failed");
      }
      result = cpkt_opcua_client_find_server_application_uri(
          client, server_url, i, uri_buffer, uri_required_size + 1u, NULL,
          &status);
    }
    if (result != CPKT_OPCUA_OK) {
      if (uri_buffer != uri_stack_buffer) {
        free(uri_buffer);
      }
      return vectis_opcua_lua_push_error(lua, result, status,
                                         "opcua find server uri");
    }

    name_buffer = name_stack_buffer;
    name_required_size = 0u;
    result = cpkt_opcua_client_find_server_application_name(
        client, server_url, i, name_buffer, sizeof(name_stack_buffer),
        &name_required_size, &status);
    if (result == CPKT_OPCUA_ERR_RANGE &&
        name_required_size > sizeof(name_stack_buffer)) {
      name_buffer = (char *)malloc(name_required_size + 1u);
      if (name_buffer == NULL) {
        if (uri_buffer != uri_stack_buffer) {
          free(uri_buffer);
        }
        return luaL_error(lua, "opcua server name allocation failed");
      }
      result = cpkt_opcua_client_find_server_application_name(
          client, server_url, i, name_buffer, name_required_size + 1u, NULL,
          &status);
    }
    if (result != CPKT_OPCUA_OK) {
      if (uri_buffer != uri_stack_buffer) {
      ...
      if (name_buffer != name_stack_buffer) {
        free(name_buffer);
      }
      return vectis_opcua_lua_push_error(lua, result, status,
                                         "opcua find server name");
    }

    lua_newtable(lua);
    lua_pushstring(lua, uri_buffer);
    lua_setfield(lua, -2, "application_uri");
    lua_pushstring(lua, name_buffer);
    lua_setfield(lua, -2, "application_name");
    lua_rawseti(lua, -2, (lua_Integer)i + 1);
    if (uri_buffer != uri_stack_buffer) {
      free(uri_buffer);
    }
...
static int vectis_opcua_lua_server_new(lua_State *lua) {
  ...
  server = (vectis_opcua_lua_server *)lua_newuserdatauv(
      lua, sizeof(vectis_opcua_lua_server), 1);
  server->server = NULL;
  server->owner = lua;
  server->access_control_ref = LUA_NOREF;
  server->methods = NULL;
  ...
  lua_newtable(lua);
  lua_setiuservalue(lua, -2, 1);
  vectis_opcua_lua_retain_owner(lua, -1, server);
...
static int vectis_opcua_lua_server_add_reference_ex(lua_State *lua) {
  cpkt_opcua_server *server;
  cpkt_opcua_node_id source_node_id;
  cpkt_opcua_node_id reference_type_id;
  ...
  source_node_id = vectis_opcua_lua_node_id_field(
      lua, 2, "source_node_id", "opcua server add_reference_ex");
  reference_type_id = vectis_opcua_lua_node_id_field(
      lua, 2, "reference_type_id", "opcua server add_reference_ex");
  target_node_id = vectis_opcua_lua_expanded_node_id_field(
      lua, 2, "target_node_id", "opcua server add_reference_ex");
  is_forward = vectis_opcua_lua_table_bool(lua, 2, "is_forward", 1);
  target_node_class = vectis_opcua_lua_table_ulong(
      lua, 2, "target_node_class", CPKT_OPCUA_NODE_CLASS_UNSPECIFIED);
  status = 0u;
  result = cpkt_opcua_server_add_reference_ex(
      server, source_node_id, reference_type_id, is_forward, target_node_id,
      target_node_class, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua server add reference ex");
  }
  lua_pushboolean(lua, 1);
  return 1;
}
...
static int vectis_opcua_lua_server_delete_reference_ex(lua_State *lua) {
  cpkt_opcua_server *server;
  cpkt_opcua_node_id source_node_id;
  cpkt_opcua_node_id reference_type_id;
  cpkt_opcua_expanded_node_id target_node_id;
  ...
  luaL_checktype(lua, 2, LUA_TTABLE);
  source_node_id = vectis_opcua_lua_node_id_field(
      lua, 2, "source_node_id", "opcua server delete_reference_ex");
  reference_type_id = vectis_opcua_lua_node_id_field(
      lua, 2, "reference_type_id", "opcua server delete_reference_ex");
  target_node_id = vectis_opcua_lua_expanded_node_id_field(
      lua, 2, "target_node_id", "opcua server delete_reference_ex");
  is_forward = vectis_opcua_lua_table_bool(lua, 2, "is_forward", 1);
  delete_bidirectional =
      vectis_opcua_lua_table_bool(lua, 2, "delete_bidirectional", 0);
  status = 0u;
  result = cpkt_opcua_server_delete_reference_ex(
      server, source_node_id, reference_type_id, is_forward, target_node_id,
      delete_bidirectional, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua server delete reference ex");
  }
  lua_pushboolean(lua, 1);
  return 1;
}
...
static int vectis_opcua_lua_client_add_reference_ex(lua_State *lua) {
  cpkt_opcua_client *client;
  cpkt_opcua_node_id source_node_id;
  cpkt_opcua_node_id reference_type_id;
  cpkt_opcua_expanded_node_id target_node_id;
  ...
  luaL_checktype(lua, 2, LUA_TTABLE);
  source_node_id = vectis_opcua_lua_node_id_field(
      lua, 2, "source_node_id", "opcua client add_reference_ex");
  reference_type_id = vectis_opcua_lua_node_id_field(
      lua, 2, "reference_type_id", "opcua client add_reference_ex");
  target_node_id = vectis_opcua_lua_expanded_node_id_field(
      lua, 2, "target_node_id", "opcua client add_reference_ex");
  is_forward = vectis_opcua_lua_table_bool(lua, 2, "is_forward", 1);
  target_node_class = vectis_opcua_lua_table_ulong(
      lua, 2, "target_node_class", CPKT_OPCUA_NODE_CLASS_UNSPECIFIED);
  status = 0u;
  result = cpkt_opcua_client_add_reference_ex(
      client, source_node_id, reference_type_id, is_forward, target_node_id,
      target_node_class, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client add reference ex");
  }
  lua_pushboolean(lua, 1);
  return 1;
}
...
static int vectis_opcua_lua_client_delete_reference_ex(lua_State *lua) {
  cpkt_opcua_client *client;
  cpkt_opcua_node_id source_node_id;
  cpkt_opcua_node_id reference_type_id;
  cpkt_opcua_expanded_node_id target_node_id;
  ...
  luaL_checktype(lua, 2, LUA_TTABLE);
  source_node_id = vectis_opcua_lua_node_id_field(
      lua, 2, "source_node_id", "opcua client delete_reference_ex");
  reference_type_id = vectis_opcua_lua_node_id_field(
      lua, 2, "reference_type_id", "opcua client delete_reference_ex");
  target_node_id = vectis_opcua_lua_expanded_node_id_field(
      lua, 2, "target_node_id", "opcua client delete_reference_ex");
  is_forward = vectis_opcua_lua_table_bool(lua, 2, "is_forward", 1);
  delete_bidirectional =
      vectis_opcua_lua_table_bool(lua, 2, "delete_bidirectional", 0);
  status = 0u;
  result = cpkt_opcua_client_delete_reference_ex(
      client, source_node_id, reference_type_id, is_forward, target_node_id,
      delete_bidirectional, &status);
  if (result != CPKT_OPCUA_OK) {
    return vectis_opcua_lua_push_error(lua, result, status,
                                       "opcua client delete reference ex");
  }
  lua_pushboolean(lua, 1);
  return 1;
}
...
static void vectis_opcua_lua_register_expanded_node_id(lua_State *lua) {
  luaL_Reg methods[] = {
      {"node_id", vectis_opcua_lua_expanded_node_id_node},
      {"namespace_uri", vectis_opcua_lua_expanded_node_id_namespace_uri},
      {"server_index", vectis_opcua_lua_expanded_node_id_server_index},
      {NULL, NULL}};
  luaL_Reg metamethods[] = {
      {"__tostring", vectis_opcua_lua_expanded_node_id_tostring},
      {"__eq", vectis_opcua_lua_expanded_node_id_eq},
      {NULL, NULL}};

  if (luaL_newmetatable(lua, VECTIS_OPCUA_EXPANDED_NODE_ID) != 0) {
    luaL_setfuncs(lua, metamethods, 0);
    lua_newtable(lua);
    luaL_setfuncs(lua, methods, 0);
    lua_setfield(lua, -2, "__index");
...
static void vectis_opcua_lua_register_client(lua_State *lua) {
  luaL_Reg methods[] = {
      ...
      {"modify_subscription", vectis_opcua_lua_client_modify_subscription},
      {"delete_subscription", vectis_opcua_lua_client_delete_subscription},
      {"monitor_value", vectis_opcua_lua_client_monitor_value},
      ...
      {"browse_children_async", vectis_opcua_lua_client_browse_children_async},
      {"call_method_async", vectis_opcua_lua_client_call_method_async},
      {"add_object_async", vectis_opcua_lua_client_add_object_async},
      {"add_variable_async", vectis_opcua_lua_client_add_variable_async},
      ...
      {"add_reference_ex", vectis_opcua_lua_client_add_reference_ex},
      {"delete_reference", vectis_opcua_lua_client_delete_reference},
      {"delete_reference_ex", vectis_opcua_lua_client_delete_reference_ex},
      ...
      {"endpoint_count", vectis_opcua_lua_client_endpoint_count},
      {"endpoint_url_at", vectis_opcua_lua_client_endpoint_url_at},
      {"endpoints", vectis_opcua_lua_client_endpoints},
      {"server_count", vectis_opcua_lua_client_server_count},
      {"server_application_uri",
       vectis_opcua_lua_client_server_application_uri},
      {"server_application_name",
       vectis_opcua_lua_client_server_application_name},
      {"find_servers", vectis_opcua_lua_client_find_servers},
...
static void vectis_opcua_lua_register_server(lua_State *lua) {
  luaL_Reg methods[] = {
      ...
      {"add_reference_ex", vectis_opcua_lua_server_add_reference_ex},
      {"delete_reference", vectis_opcua_lua_server_delete_reference},
      {"delete_reference_ex", vectis_opcua_lua_server_delete_reference_ex},
...
int luaopen_opcua(lua_State *lua) {
  luaL_Reg functions[] = {
      ...
      {"expanded_node_id", vectis_opcua_lua_expanded_node_id_parse},
      {"expanded_node_id_parse", vectis_opcua_lua_expanded_node_id_parse},
      {"expanded_node_id_local", vectis_opcua_lua_expanded_node_id_local_lua},
      {"expanded_node_id_uri", vectis_opcua_lua_expanded_node_id_uri_lua},
      {"expanded_node_id_server", vectis_opcua_lua_expanded_node_id_server_lua},
      {"expanded_node_id_server_uri",
       vectis_opcua_lua_expanded_node_id_server_uri_lua},
  ...
  vectis_opcua_lua_register_expanded_node_id(lua);
  ...
  vectis_opcua_lua_set_const(lua, "MONITORING_DISABLED",
                             CPKT_OPCUA_MONITORING_DISABLED);
  vectis_opcua_lua_set_const(lua, "MONITORING_SAMPLING",
                             CPKT_OPCUA_MONITORING_SAMPLING);
  vectis_opcua_lua_set_const(lua, "MONITORING_REPORTING",
                             CPKT_OPCUA_MONITORING_REPORTING);
  vectis_opcua_lua_set_const(lua, "DEADBAND_NONE", CPKT_OPCUA_DEADBAND_NONE);
  vectis_opcua_lua_set_const(lua, "DEADBAND_ABSOLUTE",
                             CPKT_OPCUA_DEADBAND_ABSOLUTE);
  vectis_opcua_lua_set_const(lua, "DEADBAND_PERCENT",
                             CPKT_OPCUA_DEADBAND_PERCENT);
