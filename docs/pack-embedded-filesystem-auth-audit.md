...
| Support native ordered email-code, password, TOTP, browser-session, and M2M credential flows. | `tests/unit/test_vectis_runtime.c` proves M2M continuations, browser same-origin enforcement, signed-session revocation, and Lockd cleanup. `scripts/test-e2e.sh` drives the packed email-code → password → TOTP workflow through SMTP delivery and verifies its issued credential against the protected API. Low-level enrollment, TOTP, and SMTP token primitives remain covered by `tests/unit/test_vectis_auth.c` and `tests/lua/smoke.lua`. |
