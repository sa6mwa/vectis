...
| dep:opcua | cpkt-opcua | partial | planned | yes | yes | planned | `require("opcua")` covers client/foundation plus client endpoint and FindServers discovery helpers, client/server default security byte-material configuration, server lifecycle, node IDs, expanded node IDs, scalar and array values, data values, node-model, reference, deletion, common attributes, array-dimension attributes, materialized browse/page results, browse-path translation, method registration/metadata/sync client calls, server access-control callbacks, async client read/write/browse/method/node-create callbacks, server PubSub/MQTT graph wrappers, client subscription lifecycle, value-monitor callbacks, compact event callbacks, selected event-field callbacks, full client/server array read/range families, client common attribute reads/writes, client type/view/reference remote node-management, and prepared/triggered event helpers with normal and packed local e2e against deterministic C and Lua-owned test servers; native-pointer surfaces are documented C-only exclusions, while live PubSub validation and remaining callback-heavy workflows remain. |
...
| workflow:opcua-client | OPC UA client workflows | partial | planned | yes | yes | planned | Local server-backed normal and packed e2e exists for endpoint and FindServers discovery, sync read/write, async read/write, async browse, async node creation, data-value reads, full array-family read/range coverage, array-dimension attributes, index-range writes, materialized browse/page results, method metadata reads, sync and async method calls, browse-path translation, subscription lifecycle, value-monitor callbacks, compact event callbacks, selected event-field callbacks, callback-error propagation through `client:iterate()`, basic remote object/variable creation/deletion, common attribute reads/writes, type/view/reference remote node-management, expanded-node references, and connection lifecycle; native-pointer surfaces are documented C-only exclusions, while PubSub workflows remain. |
