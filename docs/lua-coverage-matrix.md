...
| workflow:cai | AI/client/tool/MCP workflows | yes | planned | yes | no | planned | Dependency-native CAI is the primary SDK surface; Vectis service adapters for request sources, HTTP/lockd/file sinks, error mapping, and logger inheritance need an explicit ownership and streaming contract. |
