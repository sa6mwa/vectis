...
| workflow:opcua-client | OPC UA client workflows | partial | planned | yes | yes | planned | Local server-backed normal and packed e2e exists for sync read/write, async read/write, async browse, async node creation, data-value reads, full array-family read/range coverage, array-dimension attributes, index-range writes, materialized browse/page results, method metadata reads, sync and async method calls, browse-path translation, subscription lifecycle, value-monitor callbacks, compact event callbacks, selected event-field callbacks, callback-error propagation through `client:iterate()`, basic remote object/variable creation/deletion, common attribute reads/writes, type/view/reference remote node-management, expanded-node references, and connection lifecycle; native-pointer exclusion and PubSub workflows remain. |
