...
| workflow:routes-streaming | Request/response streaming route helpers | n/a | partial | yes | no | n/a | Must preserve real streaming, spooling, and file-backed naming; `server:route` supports live callback-backed `stream_source` responses and explicit file-backed `spooled_source` responses, and `server:sse` provides live SSE framing. Remaining gaps are request-body streaming policy surfaces. |
