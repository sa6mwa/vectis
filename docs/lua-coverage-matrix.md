...
| dep:opcua | cpkt-opcua | partial | planned | yes | yes | planned | `require("opcua")` covers client/foundation plus server lifecycle, node IDs, expanded node IDs, scalar and array values, data values, node-model, reference, deletion, common attributes, materialized browse/page results, browse-path translation, method registration/metadata/sync client calls, full client/server array read/range families, client common attribute reads/writes, client type/view/reference remote node-management, and prepared/triggered event helpers with normal and packed local e2e against deterministic C and Lua-owned test servers; security config, PubSub, async, subscriptions, and callback-heavy workflows remain. |
...
| workflow:opcua-client | OPC UA client workflows | partial | planned | yes | yes | planned | Local server-backed normal and packed e2e exists for sync read/write, data-value reads, full array-family read/range coverage, index-range writes, materialized browse/page results, method metadata reads, sync method calls, browse-path translation, basic remote object/variable creation/deletion, common attribute reads/writes, type/view/reference remote node-management, expanded-node references, and connection lifecycle; async, subscription, native-pointer exclusion, array-dimension, and PubSub workflows remain. |
| workflow:opcua-server | OPC UA server workflows | partial | planned | yes | yes | planned | Server lifecycle/value/node-model/reference/common-attribute APIs, data values, materialized browse/page results, method registration/callback retention, method metadata reads, browse-path translation, full array value/read families, numeric range writes, and prepared event helpers are covered by direct and packed local e2e; security config, event monitoring workflows, async-safe callback queueing, array-dimension helpers, and PubSub remain. |
