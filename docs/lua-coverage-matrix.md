...
| workflow:opcua-client | OPC UA client workflows | partial | planned | yes | yes | planned | Local server-backed normal and packed e2e exists for endpoint and FindServers discovery, sync read/write, async read/write, async browse, async node creation, data-value reads, raw history-read structured behavior, full array-family read/range coverage, array-dimension attributes, index-range writes, materialized browse/page results, method metadata reads, sync and async method calls, browse-path translation, subscription lifecycle, value-monitor callbacks, compact event callbacks, selected event-field callbacks, callback-error propagation through `client:iterate()`, basic remote object/variable creation/deletion, common attribute reads/writes, type/view/reference remote node-management, expanded-node references, and connection lifecycle; native-pointer surfaces are documented C-only exclusions, while PubSub workflows remain. |
| workflow:opcua-server | OPC UA server workflows | partial | planned | yes | yes | planned | Server lifecycle/value/node-model/reference/common-attribute APIs, default security byte-material configuration, data values, array-dimension attributes, materialized browse/page results, method registration/callback retention, method metadata reads, browse-path translation, full array value/read families, numeric range writes, prepared event helpers, and disabled PubSub/MQTT graph wrappers are covered by direct and packed local e2e; native-pointer surfaces are documented C-only exclusions, while event monitoring workflows, async-safe callback queueing, and live PubSub validation remain. |
| workflow:opcua-async | OPC UA async/subscription/PubSub workflows | partial | planned | yes | yes | planned | Callback and event-loop ownership contract is documented in `docs/lua-opcua.md`; async client read/write/browse/method/node-create calls, client subscription lifecycle, value-monitor callbacks, compact event callbacks, selected event-field callbacks, and disabled server PubSub/MQTT graph workflows are implemented with direct and packed local e2e. Live PubSub workflows remain. |
