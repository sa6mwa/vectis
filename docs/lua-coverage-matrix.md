...
| workflow:opcua-server | OPC UA server workflows | partial | planned | yes | yes | planned | Server lifecycle/value/node-model/reference/common-attribute APIs, data values, array-dimension attributes, materialized browse/page results, method registration/callback retention, method metadata reads, browse-path translation, full array value/read families, numeric range writes, prepared event helpers, and disabled PubSub/MQTT graph wrappers are covered by direct and packed local e2e; security config, event monitoring workflows, async-safe callback queueing, live PubSub validation, and native-pointer exclusions remain. |
