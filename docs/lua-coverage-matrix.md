...
| dep:libssh2 | libssh2 | planned | partial | yes | no | n/a | `vectis.ssh.exec`, libssh2-backed SFTP/SCP file upload/download, one-shot SFTP filesystem operations, and stateful SFTP session/file/directory receivers exist; raw SSH channels and advanced host-key workflows remain missing. |
...
| workflow:auth | Native auth, OAuth2/OIDC, WebDAV keys | n/a | partial | yes | yes | yes | Core auth functions, Basic Authorization formatting, native/callback providers, auth routes, email tokens, OAuth2/OIDC flows, and WebDAV-key issuance are exposed and documented; higher-level Lua admin/browser DX still needs polish. |
...
| workflow:http-client | Downstream HTTP client | yes | yes | yes | yes | planned | JSON helpers, simple non-JSON verbs including HEAD, form bodies, multipart, file upload/download, retry/proxy/TLS pass-through, and reusable client defaults exist. |
...
| workflow:sftp-libssh2 | libssh2-backed SFTP | planned | yes | yes | yes | n/a | `vectis.ssh.sftp_upload_file`, `sftp_download_file`, `sftp_stat`, `sftp_mkdir`, `sftp_remove`, `sftp_rmdir`, `sftp_rename`, `sftp_chmod`, and `sftp_open` expose one-shot plus stateful open/read/write/stat/directory iteration handles. |
