...
| dep:sus | cpkt SUS / whisper | partial | planned | yes | yes | planned | Raw metadata, catalog lookup, path/cache open error handling, model handles, offline cache status callbacks, model-created transcriber handles, PCM transcription methods, and transcriber callback registration are exposed, documented in `docs/lua-sus.md`, and packed-tested; segmented decoder/VOX interop and live model coverage remain. |
