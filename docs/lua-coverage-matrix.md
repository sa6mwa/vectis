...
| dep:opcua | cpkt-opcua | partial | planned | yes | yes | planned | `require("opcua")` covers client/foundation with normal and packed local e2e against the deterministic C test server; server, async, subscriptions, PubSub remain. |
...
| workflow:opcua-client | OPC UA client workflows | yes | planned | yes | yes | planned | Local server-backed normal and packed e2e exists; Vectis workflow helper is not designed yet. |
