...
| dep:openssl | OpenSSL | partial | partial | yes | yes | n/a | `openssl` exposes version, SHA-256, general EVP digest/HMAC, and CSPRNG helpers; `vectis.cert` owns certificate workflows; broader signing/verification primitives remain. |
...
| workflow:auth | Native auth, OAuth2/OIDC, WebDAV keys | n/a | partial | yes | yes | yes | Core auth functions, native/callback providers, auth routes, email tokens, OAuth2/OIDC flows, and WebDAV-key issuance are exposed and documented; higher-level Lua admin/browser DX still needs polish. |
