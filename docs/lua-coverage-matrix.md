...
| workflow:ssh-exec | SSH command execution | planned | yes | yes | no | n/a | `vectis.ssh.exec` and reusable `vectis.ssh.open(...):exec(...)` receiver calls exist; lower-level dependency-native libssh2 channels remain missing. |
