...
| dep:opcua | cpkt-opcua | partial | planned | yes | yes | planned | `require("opcua")` covers client/foundation plus server lifecycle, node IDs, expanded node IDs, scalar and array values, data values, node-model, reference, deletion, common attributes, array-dimension attributes, materialized browse/page results, browse-path translation, method registration/metadata/sync client calls, async client read/write/browse/method/node-create callbacks, client subscription lifecycle, value-monitor callbacks, compact event callbacks, selected event-field callbacks, full client/server array read/range families, client common attribute reads/writes, client type/view/reference remote node-management, and prepared/triggered event helpers with normal and packed local e2e against deterministic C and Lua-owned test servers; security config, PubSub, native-pointer exclusions, and remaining callback-heavy workflows remain. |
...
| workflow:opcua-server | OPC UA server workflows | partial | planned | yes | yes | planned | Server lifecycle/value/node-model/reference/common-attribute APIs, data values, array-dimension attributes, materialized browse/page results, method registration/callback retention, method metadata reads, browse-path translation, full array value/read families, numeric range writes, and prepared event helpers are covered by direct and packed local e2e; security config, event monitoring workflows, async-safe callback queueing, and PubSub remain. |
| workflow:opcua-async | OPC UA async/subscription/PubSub workflows | partial | planned | yes | yes | planned | Callback and event-loop ownership contract is documented in `docs/lua-opcua.md`; async client read/write/browse/method/node-create calls, client subscription lifecycle, value-monitor callbacks, compact event callbacks, and selected event-field callbacks are implemented with direct and packed local e2e. PubSub workflows remain. |
