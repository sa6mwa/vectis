...
| dep:libssh2 | libssh2 | planned | partial | yes | no | n/a | `vectis.ssh.exec`, libssh2-backed SFTP/SCP file upload/download, and one-shot SFTP filesystem operations exist; raw sessions, channels, directory iteration, open/read/write handles, and advanced host-key workflows are missing. |
...
| workflow:sftp-libssh2 | libssh2-backed SFTP | planned | partial | yes | no | n/a | `vectis.ssh.sftp_upload_file`, `sftp_download_file`, `sftp_stat`, `sftp_mkdir`, `sftp_remove`, `sftp_rmdir`, `sftp_rename`, and `sftp_chmod` expose one-shot operations; stateful open/read/write/list handles remain. |
