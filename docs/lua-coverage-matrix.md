...
| dep:opcua | cpkt-opcua | partial | planned | yes | yes | planned | `require("opcua")` covers client/foundation plus server lifecycle, scalar and array values, data values, node-model, reference, deletion, common attributes, materialized browse/page results, browse-path translation, method registration/metadata/sync client calls, full client/server array read/range families, and prepared/triggered event helpers with normal and packed local e2e against deterministic C and Lua-owned test servers; security config, PubSub, async, subscriptions, and callback-heavy workflows remain. |
...
| workflow:opcua-client | OPC UA client workflows | partial | planned | yes | yes | planned | Local server-backed normal and packed e2e exists for sync read/write, data-value reads, full array-family read/range coverage, index-range writes, materialized browse/page results, method metadata reads, sync method calls, browse-path translation, basic remote object/variable creation/deletion, and connection lifecycle; broader attributes, async, subscription, and advanced remote node-management workflows remain. |
