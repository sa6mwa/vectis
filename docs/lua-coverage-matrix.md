...
| dep:opcua | cpkt-opcua | partial | planned | yes | yes | planned | `require("opcua")` covers client/foundation plus server lifecycle, node IDs, expanded node IDs, scalar and array values, data values, node-model, reference, deletion, common attributes, array-dimension attributes, materialized browse/page results, browse-path translation, method registration/metadata/sync client calls, async client read/write/method callbacks, client subscription lifecycle, value-monitor callbacks, compact event callbacks, selected event-field callbacks, full client/server array read/range families, client common attribute reads/writes, client type/view/reference remote node-management, and prepared/triggered event helpers with normal and packed local e2e against deterministic C and Lua-owned test servers; security config, PubSub, async browse/node creation, and remaining callback-heavy workflows remain. |
...
| workflow:opcua-client | OPC UA client workflows | partial | planned | yes | yes | planned | Local server-backed normal and packed e2e exists for sync read/write, async read/write, data-value reads, full array-family read/range coverage, array-dimension attributes, index-range writes, materialized browse/page results, method metadata reads, sync and async method calls, browse-path translation, subscription lifecycle, value-monitor callbacks, compact event callbacks, selected event-field callbacks, callback-error propagation through `client:iterate()`, basic remote object/variable creation/deletion, common attribute reads/writes, type/view/reference remote node-management, expanded-node references, and connection lifecycle; async browse, async node creation, native-pointer exclusion, and PubSub workflows remain. |
...
| workflow:opcua-async | OPC UA async/subscription/PubSub workflows | partial | planned | yes | yes | planned | Callback and event-loop ownership contract is documented in `docs/lua-opcua.md`; async client read/write/method calls, client subscription lifecycle, value-monitor callbacks, compact event callbacks, and selected event-field callbacks are implemented with direct and packed local e2e. Async browse, async node creation, and PubSub workflows remain. |
