...
| workflow:lockd-queue | lockd queue and consumer workflows | yes | partial | yes | yes | n/a | Raw lockdc plus `vectis.lockd.enqueue_json` and `with_dequeued_json`; retry-oriented helper DX remains only where raw lockdc is too noisy. |
