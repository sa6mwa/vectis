...
| workflow:routes-streaming | Request/response streaming route helpers | n/a | yes | yes | no | n/a | `server:upload` provides true request-body streaming through a bounded upload reader and Lua chunk callbacks; `server:route` supports live callback-backed `stream_source` responses and explicit file-backed `spooled_source` responses; `server:sse` provides live SSE framing. |
...
| workflow:ssh-exec | SSH command execution | planned | yes | yes | no | n/a | `vectis.ssh.exec` and reusable `vectis.ssh.open(...):exec(...)` receiver calls exist; raw dependency-native libssh2 channels remain missing. |
...
| workflow:sftp-libssh2 | libssh2-backed SFTP | planned | yes | yes | yes | yes | `vectis.ssh.sftp_upload_file`, `sftp_download_file`, `sftp_stat`, `sftp_mkdir`, `sftp_remove`, `sftp_rmdir`, `sftp_rename`, `sftp_chmod`, `sftp_open`, and `vectis.ssh.open(...):sftp_*` expose one-shot plus reusable SSH and stateful SFTP open/read/write/stat/directory iteration handles, including opt-in SSH/SFTP e2e coverage. |
