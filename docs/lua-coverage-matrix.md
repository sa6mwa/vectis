...
| dep:sus | cpkt SUS / whisper | partial | planned | yes | yes | planned | Raw metadata, catalog lookup, path/cache open error handling, model handles, offline cache status callbacks, process-wide log sink configuration, model-created transcriber handles, PCM transcription methods, and transcriber callback registration are exposed, documented in `docs/lua-sus.md`, and packed-tested; segmented decoder/VOX interop and live model coverage remain. |
...
| workflow:sus | Whisper/transcription workflows | partial | planned | yes | yes | planned | Catalog, cache-control paths, model-created transcribers, PCM transcription methods, and callback registration are exposed; audio decoder/VOX segmented transcription and live/local model tests remain. |
