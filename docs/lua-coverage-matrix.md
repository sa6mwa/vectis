...
| dep:opcua | cpkt-opcua | partial | planned | yes | yes | planned | `require("opcua")` covers client/foundation plus server lifecycle, scalar and array values, data values, node-model, reference, deletion, common attributes, browse-path translation, array/range attributes, and prepared/triggered event helpers with normal and packed local e2e against deterministic C and Lua-owned test servers; methods, browse callbacks/pagination, security config, PubSub, async, subscriptions, and callback-heavy workflows remain. |
...
| workflow:opcua-client | OPC UA client workflows | partial | planned | yes | yes | planned | Local server-backed normal and packed e2e exists for sync read/write, data-value reads, integer array/range reads, index-range writes, browse-path translation, basic remote object/variable creation/deletion, and connection lifecycle; broader attributes, full array-family parity, async, browse callbacks/pagination, method-call, subscription, and advanced remote node-management workflows remain. |
| workflow:opcua-server | OPC UA server workflows | partial | planned | yes | yes | planned | Server lifecycle/value/node-model/reference/common-attribute APIs, data values, browse-path translation, full array value/read families, numeric range writes, and prepared event helpers are covered by direct and packed local e2e; methods, browse callbacks/pagination, security config, event monitoring workflows, and PubSub remain. |
