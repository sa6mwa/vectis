...
`vectis.log` is a small logging DX layer over the raw `pslog` Lua module. It
does not replace `pslog`; raw access remains available as `vectis.log.raw`.
...
the raw pslog option table. Other options are passed through to
`pslog.new_json(opts)`.
...
`log.error_fields(err)` returns a key/value array suitable for advanced raw
