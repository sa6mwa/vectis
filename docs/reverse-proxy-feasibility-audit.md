...
connection reuse after cancellation, and BSD kqueue behavior remain
open.
...
connection reuse, other target bundles, or a production admission reserve.
...
easy handle. The HTTP/2 peer observes `RST_STREAM` promptly; the underlying
TLS socket need not close because libcurl may keep it for reuse. Five serial
runs passed. This establishes cancellation of one active HTTP/2 stream, but
does not establish simultaneous cancellation of paused streams or subsequent
reuse of that connection.
