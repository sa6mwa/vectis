...
WebSocket lookup cannot
become raw fallback. A static directory registered before the overlapping
ordinary and proxy routes serves its file over cleartext and TLS. POST gets
...
path exception. Production admission still needs live-upload overlap and
complete header framing.
