...
two phases. It does not prove a reset while Kore has queued but unwritten
response bytes, a TLS upstream reset, or the production decision for an
upstream failure after headers are parsed but before any downstream byte is
committed. That boundary needs explicit output accounting so a `502` is
sent only when it cannot conflict with a partially written response.

A separate slow-consumer SSE fixture sets a small downstream receive buffer,
then produces bounded 4 KiB chunks from a 64 MiB logical generator. The
worker retains only one bounded Kore netbuf while the client stops reading.
The test waits until `net_send_flush()` leaves that netbuf queued, resets the
client, and confirms in Kore's disconnect callback that the netbuf is still
queued. The proxy cancels libcurl once and the upstream producer observes
its socket close. Five serial Linux runs pass; the existing queue bound and
pump-count assertions still hold. This proves cleanup with a pending
cleartext Kore netbuf and real backpressure. It does not force the reset
inside `net_send_flush()` itself or cover an equivalent downstream TLS
queue abort. Those callback phases remain explicit lifecycle gates.
...
The framer is a probe sink: it does not feed libcurl, pause on upstream
backpressure, reject malformed framing without assertions, or implement a
general trailer/header policy. Those remain integration gates.
