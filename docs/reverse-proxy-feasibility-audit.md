...
skips WebSocket matching only
for an invalid ordinary path, and enters raw fallback only for that path
condition or decode failure. Errors from a valid-path WebSocket lookup cannot
become raw fallback. Production admission still needs live-upload and static
overlap plus complete header framing.
