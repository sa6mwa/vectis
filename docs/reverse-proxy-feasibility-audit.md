...
Its test-only `connection->write` wrapper delegates normal writes to Kore's
`kore_tls_write`, then returns an error when the first body netbuf is queued.
...
when the writer reports failure; the error is injected before `SSL_write`, so
an actual OpenSSL write-call failure and cross-direction retries remain open.
