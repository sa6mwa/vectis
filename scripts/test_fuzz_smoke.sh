#!/usr/bin/env bash
set -euo pipefail

script_dir=$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)
repo_root=$(CDPATH='' cd -- "$script_dir/.." && pwd)
build_dir=${1:-"$repo_root/build/fuzz"}
tmp_dir=$(mktemp -d "$build_dir/fuzz-smoke.XXXXXXXX")
unset LD_LIBRARY_PATH LD_PRELOAD LD_AUDIT

cleanup() {
  rm -rf "$tmp_dir"
}
trap cleanup EXIT INT TERM

value() {
  sed -n "s/^$1=//p" | tail -1
}

afl_showmap=$("$script_dir/cpkt-aflpp.sh" discover | value afl_showmap)
python3 "$script_dir/test_runtime_contract.py" "$build_dir"
json_target="$build_dir/tests/fuzz/vectis_fuzz_json_validate"
kore_target="$build_dir/tests/fuzz/vectis_fuzz_kore_bridge"
proxy_framing_target="$build_dir/tests/fuzz/vectis_fuzz_proxy_framing"
proxy_headers_target="$build_dir/tests/fuzz/vectis_fuzz_proxy_headers"
proxy_ws_target="$build_dir/tests/fuzz/vectis_fuzz_proxy_ws_handshake"
proxy_url_target="$build_dir/tests/fuzz/vectis_fuzz_proxy_url"
proxy_director_target="$build_dir/tests/fuzz/vectis_fuzz_proxy_director"
proxy_http_target="$build_dir/tests/fuzz/vectis_fuzz_proxy_http"
proxy_response_target="$build_dir/tests/fuzz/vectis_fuzz_proxy_response"
proxy_local_target="$build_dir/tests/fuzz/vectis_fuzz_proxy_local"

if [ ! -x "$json_target" ]; then
  echo "missing fuzz target: $json_target" >&2
  exit 1
fi
if [ ! -x "$kore_target" ]; then
  echo "missing fuzz target: $kore_target" >&2
  exit 1
fi
if [ ! -x "$proxy_framing_target" ]; then
  echo "missing fuzz target: $proxy_framing_target" >&2
  exit 1
fi
if [ ! -x "$proxy_headers_target" ]; then
  echo "missing fuzz target: $proxy_headers_target" >&2
  exit 1
fi
if [ ! -x "$proxy_ws_target" ]; then
  echo "missing fuzz target: $proxy_ws_target" >&2
  exit 1
fi
if [ ! -x "$proxy_url_target" ]; then
  echo "missing fuzz target: $proxy_url_target" >&2
  exit 1
fi
if [ ! -x "$proxy_director_target" ]; then
  echo "missing fuzz target: $proxy_director_target" >&2
  exit 1
fi
if [ ! -x "$proxy_http_target" ]; then
  echo "missing fuzz target: $proxy_http_target" >&2
  exit 1
fi
if [ ! -x "$proxy_response_target" ]; then
  echo "missing fuzz target: $proxy_response_target" >&2
  exit 1
fi
if [ ! -x "$proxy_local_target" ]; then
  echo "missing fuzz target: $proxy_local_target" >&2
  exit 1
fi

mkdir -p "$tmp_dir"
printf '%s\n' '{"ok":true}' >"$tmp_dir/json.seed"
AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/json.map" -- "$json_target" \
  <"$tmp_dir/json.seed" >/dev/null

for seed in "$repo_root"/tests/fuzz/corpus/kore_bridge/*; do
  seed_name=$(basename -- "$seed")
  AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/kore-$seed_name.map" -- \
    "$kore_target" <"$seed" >/dev/null
done

for seed in "$repo_root"/tests/fuzz/corpus/proxy_framing/*; do
  seed_name=$(basename -- "$seed")
  AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/proxy-framing-$seed_name.map" -- \
    "$proxy_framing_target" <"$seed" >/dev/null
done

for seed in "$repo_root"/tests/fuzz/corpus/proxy_headers/*; do
  seed_name=$(basename -- "$seed")
  AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/proxy-headers-$seed_name.map" -- \
    "$proxy_headers_target" <"$seed" >/dev/null
done

for seed in "$repo_root"/tests/fuzz/corpus/proxy_ws_handshake/*; do
  seed_name=$(basename -- "$seed")
  AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/proxy-ws-$seed_name.map" -- \
    "$proxy_ws_target" <"$seed" >/dev/null
done

for seed in "$repo_root"/tests/fuzz/corpus/proxy_url/*; do
  seed_name=$(basename -- "$seed")
  AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/proxy-url-$seed_name.map" -- \
    "$proxy_url_target" <"$seed" >/dev/null
done

for seed in "$repo_root"/tests/fuzz/corpus/proxy_director/*; do
  seed_name=$(basename -- "$seed")
  AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/proxy-director-$seed_name.map" -- \
    "$proxy_director_target" <"$seed" >/dev/null
done

for seed in "$repo_root"/tests/fuzz/corpus/proxy_http/*; do
  seed_name=$(basename -- "$seed")
  AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/proxy-http-$seed_name.map" -- \
    "$proxy_http_target" <"$seed" >/dev/null
done

for seed in "$repo_root"/tests/fuzz/corpus/proxy_response/*; do
  seed_name=$(basename -- "$seed")
  AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/proxy-response-$seed_name.map" -- \
    "$proxy_response_target" <"$seed" >/dev/null
done

for seed in "$repo_root"/tests/fuzz/corpus/proxy_local/*; do
  seed_name=$(basename -- "$seed")
  AFL_QUIET=1 "$afl_showmap" -o "$tmp_dir/proxy-local-$seed_name.map" -- \
    "$proxy_local_target" <"$seed" >/dev/null
done
