...
assert_lua_coverage_matrix_contract() {
  ...
  assert_contains "$repo_root/TODO.md" '\[ \] Add Lua SSE route helpers'
  assert_contains "$repo_root/TODO.md" 'live `stream_source` responses'
  ...
  assert_contains "$matrix" 'SSE helpers remain a separate missing surface'
  ...
  assert_contains "$matrix" 'dependency-native SSH channels and advanced host-key workflows remain missing'
  ...
  assert_contains "$lua_index" 'same identity rule applies to the other workflow modules'
