...
assert_lockdc_lua_runtime_contract() {
  ...
  assert_contains "$repo_root/examples/lua/lockd_queue.lua" 'vectis\.lockd\.encode_json'
...
assert_lua_coverage_matrix_contract() {
  ...
  assert_contains "$repo_root/docs/lua-http.md" 'and `head` set the HTTP method'
  assert_contains "$matrix" 'simple non-JSON verbs including HEAD'
  ...
  assert_contains "$repo_root/TODO.md" 'raw buffered zlib Lua facade'
  ...
  assert_contains "$repo_root/docs/lua-zlib.md" 'explicitly buffered'
  assert_contains "$repo_root/docs/api.md" '\[Lua zlib\]\(lua-zlib\.md\)'
  assert_contains "$matrix" 'buffered deflate/inflate'
