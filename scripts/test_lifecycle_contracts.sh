...
assert_lua_coverage_matrix_contract() {
  ...
  assert_contains "$matrix" 'server lifecycle, value, node-model, reference, deletion, and common attribute APIs'
  ...
  assert_contains "$matrix" 'Server lifecycle/value/node-model/reference/common-attribute APIs are covered by direct and packed local e2e'
